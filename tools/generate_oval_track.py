...
four artifacts guaranteed to share one geometry:
  ...
  3. maps/osm/oval_track_spawn.json   ego start pose -> OvalTrackScenario
  4. src/action/fake_planner/maneuvers/oval_track.json  -> the lane centreline to drive

(3) exists so the CARLA scenario reads the drive lane's start pose instead of carrying its own
copy of it: the ego and the maneuver then cannot disagree about which lane the car is in.
...
Usage:
    python3 tools/generate_oval_track.py            # writes all four artifacts
    python3 tools/generate_oval_track.py --check    # verify only, write nothing

--check also fails if the tracked maneuver JSON on disk has drifted from what this script would
emit (compared as data: pre-commit's clang-format re-wraps .json files), which is what makes
"do not hand-edit, regenerate" enforceable.
"""
...
# on the mesh. The CARLA scenario reads the resulting start pose out of SPAWN_OUT, so this is the
# only place the drive lane is chosen.
DRIVE_LANE = 1
...
SPAWN_OUT = os.path.join(REPO, "maps", "osm", "oval_track_spawn.json")
...
def build_spawn():
    """
    The ego spawn pose for OvalTrackScenario: the same start the maneuver is authored from.

    Emitted as its own artifact so the scenario has no copy of these numbers to keep in step --
    the drive lane is chosen once, here, by DRIVE_LANE.
    """
    r = lane_centre_radius(DRIVE_LANE)
    return {
        "_comment": (
            "Generated by tools/generate_oval_track.py -- do not hand-edit. Ego start pose for "
            "the oval track, in OpenDRIVE/ROS coordinates (CARLA negates Y when it reads this). "
            "Matches the 'start' pose in fake_planner/maneuvers/oval_track.json."
        ),
        "x": -CAP_CENTRE_X,
        "y": -r,
        "yaw": 0.0,
        "lane": DRIVE_LANE,
        "lane_centre_radius_m": r,
...
def verify(nodes, ways, rels):
    ...
    # Envelope, asserted as the relationship rather than the numbers it currently works out to,
    # so changing INNER_RADIUS or STRAIGHT_LENGTH does not fail geometry that is perfectly correct.
    check(
        "envelope == %.0f x %.0f m" % (2 * EXTENT_X, 2 * EXTENT_Y),
        abs(EXTENT_X - (CAP_CENTRE_X + OUTER_RADIUS)) < 1e-9
        and abs(EXTENT_Y - OUTER_RADIUS) < 1e-9
        and abs(OUTER_RADIUS - (INNER_RADIUS + NUM_LANES * LANE_WIDTH)) < 1e-9
        and abs(CAP_CENTRE_X - STRAIGHT_LENGTH / 2.0) < 1e-9,
    )
    ...
    # The maneuver is absolute, so a spawn pose that drifts from its start puts the car in a
    # different lane from the path. Both come from DRIVE_LANE, so this checks the wiring.
    spawn = build_spawn()
    check(
        "ego spawn matches the maneuver start",
        abs(spawn["x"] - maneuver["start"]["x"]) < 1e-12
        and abs(spawn["y"] - maneuver["start"]["y"]) < 1e-12
        and abs(spawn["yaw"] - maneuver["start"]["yaw"]) < 1e-12,
        "(spawn %.2f, %.2f vs maneuver %.2f, %.2f)"
        % (spawn["x"], spawn["y"], maneuver["start"]["x"], maneuver["start"]["y"]),
    )
    check(
        "ego spawn sits on the drive lane centreline",
        abs(dist_to_spine(spawn["x"], spawn["y"]) - r_drive) < 1e-9,
    )
...
def maneuver_json(maneuver):
    """The bytes written to MANEUVER_OUT. clang-format re-wraps these on commit, which is fine."""
    return json.dumps(maneuver, indent=2) + "\n"


def check_no_drift(maneuver):
    """
    True if the tracked maneuver on disk carries the same data this script would emit.

    Compared as parsed JSON rather than as bytes on purpose: pre-commit runs clang-format over
    .json too, so the committed file is re-wrapped after generation and would never match
    json.dumps byte-for-byte. Formatting belongs to the formatter; what must not drift is content.

    Only MANEUVER_OUT is checked -- the .osm/.xodr/spawn artifacts live under the git-ignored
    maps/, so there is no tracked copy of them to drift. Without this, "generated -- do not
    hand-edit" is a comment rather than a guarantee.
    """
    label = "tracked maneuver JSON matches the generator"
    try:
        with open(MANEUVER_OUT) as f:
            have = json.load(f)
    except (OSError, ValueError) as e:
        print("  %-46s FAIL (%s)" % (label, e))
        return False
    ok = have == maneuver
    hint = (
        ""
        if ok
        else "(run `python3 tools/generate_oval_track.py` and commit the result)"
    )
    print("  %-46s %s %s" % (label, "PASS" if ok else "FAIL", hint))
    return ok
...
def main():
    ...
    if args.check:
        # The tracked artifact is only comparable in --check mode; a write run makes it match by
        # construction.
        ok = check_no_drift(maneuver) and ok
        if not ok:
    ...
    with open(XODR_OUT, "w") as f:
        f.write(xodr)
    with open(SPAWN_OUT, "w") as f:
        json.dump(spawn, f, indent=2)
        f.write("\n")
    with open(MANEUVER_OUT, "w") as f:
        f.write(maneuver_json(maneuver))
    print(
        "Wrote:\n  %s\n  %s\n  %s\n  %s" % (OSM_OUT, XODR_OUT, SPAWN_OUT, MANEUVER_OUT)
    )
