...
def main():
    ...
    lanelets = {
        rid: (t, m) for rid, (t, m) in rels.items() if t.get("type") == "lanelet"
    }
    regs = {
        rid: (t, m)
    ...
    # ---- 1. structure ----
    check(
        bool(nodes) and bool(ways) and bool(lanelets),
        "map loads (nodes/ways/lanelets)",
        f"{len(nodes)}/{len(ways)}/{len(lanelets)}",
    )
    check(
        any(e.tag == "annotation" for e in ET.parse(MAP).getroot()),
    ...
    for rid, (t, m) in lanelets.items():
        ...
        for role, ty, rf in m:
            if role == "successor":
                ...
                pred[rid].append(rf)
            elif role in ("adjacentLeft", "adjacentRight"):
                adj[rid].append(rf)
    ...
    A = rid_of["ringroad_uv_exit"]
    B = rid_of["uvway_west_out"]
    C = rid_of["uvway_east_in"]
    D = rid_of["ringroad_uv_enter"]
    ...
    while q:
        ...
        for x in succ[r]:
            if x not in seen:
                ...
                q.append(x)
    check(
        all(x in ap for x in ring_fwd),
        "loop forward is a closed ring",
        f"reachable {len(ap)}/{len(ring_fwd)} fwd lanelets",
    )
    check(
        all(x not in ap for x in ring_rev),
        "loop directions are separated",
    ...
    while q:
        ...
        for x in succ[r]:
            if x not in seen:
                ...
                q.append(x)
    check(
        all(x in ap2 for x in ring_rev),
        "loop reverse is a closed ring",
    ...
    check(
        reachable(C, E) and reachable(C, D),
    ...
    check(
        best == r0,
        "nearest-lanelet finds correct lanelet at sample point",
        f"got {name_of[best]}",
    )
    ...
    check(
        wavg > 3.2 and wmax <= LANE_WIDTH * 1.02 and wmin > 1.5,
    ...
    # speed limits parse
    check(
        all(v in (40.0, 50.0) for v in spd.values()),
        "speed limits 40/50 km/h parse",
