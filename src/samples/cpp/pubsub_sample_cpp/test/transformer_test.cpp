...
public:
  void SetUp(int expected_msgs)
  {
    ...
    spy = std::make_shared<PubSubSpy<pubsub_sample_cpp::msg::FilteredArray>>(
      helper_node, executor, "filtered", Transformer::ADVERTISING_FREQ, expected_msgs);

    unfiltered_pub = helper_node->create_publisher<pubsub_sample_cpp::msg::Unfiltered>(
...
protected:
  ...
  std::shared_ptr<PubSubSpy<pubsub_sample_cpp::msg::FilteredArray>> spy;
  rclcpp::Publisher<pubsub_sample_cpp::msg::Unfiltered>::SharedPtr unfiltered_pub;
};
...
TEST_F(TransformerTest, PublishMultipleBuffers)
{
  ...
  for (int i = 0; i < 2 * Transformer::BUFFER_CAPACITY; i++) {
    ...
    msg.valid = true;
    msg.timestamp = 0;
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "%d\n", test_node->count_subscribers("unfiltered"));
    unfiltered_pub->publish(msg);
    executor->spin_some(std::chrono::milliseconds(10));
