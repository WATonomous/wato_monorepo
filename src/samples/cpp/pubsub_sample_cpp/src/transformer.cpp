...
: Node("transformer")
{
  raw_sub_ = this->create_subscription<pubsub_sample_cpp::msg::Unfiltered>(
    "unfiltered", ADVERTISING_FREQ,
    std::bind(
      &Transformer::unfiltered_callback, this,
      std::placeholders::_1));
  transform_pub_ =
    this->create_publisher<pubsub_sample_cpp::msg::FilteredArray>("filtered", ADVERTISING_FREQ);
...
void Transformer::unfiltered_callback(const pubsub_sample_cpp::msg::Unfiltered::SharedPtr msg)
{
  auto filtered_msg = pubsub_sample_cpp::msg::Filtered();
...
bool Transformer::filtered_transformer(
  const pubsub_sample_cpp::msg::Unfiltered::SharedPtr unfiltered,
  pubsub_sample_cpp::msg::Filtered & filtered)
{
...
void Transformer::publish_buffer(const pubsub_sample_cpp::msg::Filtered & msg)
{
  buffer_.push_back(msg);
  // Publish processed data when the buffer reaches its capacity
  if (buffer_.size() == BUFFER_CAPACITY) {
    pubsub_sample_cpp::msg::FilteredArray filtered_msgs;
