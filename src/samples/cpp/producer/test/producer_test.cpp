...
protected:
  void SetUp(
    std::function<
      void(const producer::msg::Unfiltered::SharedPtr)
    > callback,
    std::shared_ptr<Producer> producer_node,
    int trigger_count)
  {
    auto test_node = rclcpp::Node::make_shared("test_node");
    auto subscriber = test_node->create_subscription<producer::msg::Unfiltered>(
...
TEST_F(ProducerTest, PublishSingleMessage)
{
  ...
  auto callback =
    ...
    const producer::msg::Unfiltered::SharedPtr msg) -> void {
      msgs_received++;
