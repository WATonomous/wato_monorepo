...
def generate_launch_description():
    ...
    if '$' in params['carla']['host']: 
        hostname = os.environ.get(params['carla']['host'].split()[1], params['carla']['host'].split()[2])
    else:
    ...
    timeout_arg = DeclareLaunchArgument('timeout', default_value=str(params['carla']['timeout']))
    sync_mode_arg = DeclareLaunchArgument('synchronous_mode', default_value=str(params['carla']['synchronous_mode']))
    sync_mode_wait_arg = DeclareLaunchArgument('synchronous_mode_wait_for_vehicle_control_command', default_value=str(params['carla']['synchronous_mode_wait_for_vehicle_control_command']))
    fixed_delta_arg = DeclareLaunchArgument('fixed_delta_seconds', default_value=str(params['carla']['fixed_delta_seconds']))
    town_arg = DeclareLaunchArgument('town', default_value=params['carla']['town'])

    # Carla ego vehicle args
    objects_definition_arg = DeclareLaunchArgument('objects_definition_file', default_value=str(get_package_share_directory('carla_config') + '/config/objects.json'))
    role_name_arg = DeclareLaunchArgument('role_name', default_value=params['carla']['ego_vehicle']['role_name'])
    spawn_point_arg = DeclareLaunchArgument('spawn_point', default_value='None')
    spawn_sensors_only_arg = DeclareLaunchArgument('spawn_sensors_only', default_value='False')

    # Ackermann control args
    control_loop_rate_arg = DeclareLaunchArgument('control_loop_rate', default_value=str(params['carla']['ackermann_control']['control_loop_rate']))
