...
class MPC_Bridge_Node : public rclcpp::Node {
  ...
  rclcpp::Publisher<embedded_msgs::msg::SteeringAngleCAN>::SharedPtr
      steeringPub;
  rclcpp::Publisher<path_planning_msgs::msg::AckermannDrive>::SharedPtr
      carlaPub;

public:
  MPC_Bridge_Node() : Node("carla_mpc_bridge") {
    // Declare Parameters
    this->declare_parameter("mpc_output_topic", "/mpc_output");
    this->declare_parameter("steering_publisher_topic", "/steering_data");

    // Get Parameters
    std::string mpc_output_topic =
        this->get_parameter("mpc_output_topic").as_string();
    std::string steering_publisher_topic =
        this->get_parameter("steering_publisher_topic").as_string();
    ...
    do {
      role_name = parameters_client->get_parameters({"role_name"})[0]
                      .get_value<std::string>();
      std::this_thread::sleep_for(std::chrono::seconds(1));
    } while (role_name == "");

    // Subscribe to vehicle status
    rclcpp::Subscription<path_planning_msgs::msg::CarlaEgoVehicleStatus>::
        SharedPtr steeringAngleSub;
    steeringAngleSub = this->create_subscription<
        path_planning_msgs::msg::CarlaEgoVehicleStatus>(
        "/carla/" + role_name + "/vehicle_status", 2,
        std::bind(&MPC_Bridge_Node::publish_steering, this, _1));
    ...
    mpcSub = this->create_subscription<path_planning_msgs::msg::MPCOutput>(
        mpc_output_topic, 2,
        std::bind(&MPC_Bridge_Node::mpc_to_carla, this, _1));

    rclcpp::Subscription<path_planning_msgs::msg::Environment>::SharedPtr
        envSub;
    envSub = this->create_subscription<path_planning_msgs::msg::Environment>(
        "/path_planning/environment", 2,
        std::bind(&MPC_Bridge_Node::env_callback, this, _1));

    this->steeringPub =
        this->create_publisher<embedded_msgs::msg::SteeringAngleCAN>(
            steering_publisher_topic, 1);
    this->carlaPub =
        this->create_publisher<path_planning_msgs::msg::AckermannDrive>(
            "/carla/" + role_name + "/ackermann_cmd", 1);
  }

private:
  void publish_steering(
      path_planning_msgs::msg::CarlaEgoVehicleStatus::SharedPtr vehicle) {
