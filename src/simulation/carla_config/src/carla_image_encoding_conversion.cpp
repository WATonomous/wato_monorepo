...
class Image_Encoding_Conversion : public rclcpp::Node {
 public:
  Image_Encoding_Conversion() : Node("image_encoding_conversion_node") {
    // Wait for service
    auto parameters_client =
        std::make_shared<rclcpp::SyncParametersClient>(this, "set_initial_pose");
    ...
    // Declare Parameters
    this->declare_parameter("camera_out_center_topic",
                            "/carla/" + role_name + "/rgb_center/image_bgr");
    this->declare_parameter("camera_out_right_topic",
                            "/carla/" + role_name + "/rgb_right/image_bgr");
    this->declare_parameter("camera_out_left_topic", "/carla/" + role_name + "/rgb_left/image_bgr");

    // Get Parameters
    std::string camera_out_center_topic =
        this->get_parameter("camera_out_center_topic").as_string();
    ...
    this->imagePubCenter =
        this->create_publisher<sensor_msgs::msg::Image>(camera_out_center_topic, 10);
    imageSubCenter = this->create_subscription<sensor_msgs::msg::Image>(
        "/carla/" + role_name + "/rgb_center/image", 10,
        std::bind(&Image_Encoding_Conversion::update_image_center, this, _1));
    this->imagePubRight =
        this->create_publisher<sensor_msgs::msg::Image>(camera_out_right_topic, 10);
    imageSubRight = this->create_subscription<sensor_msgs::msg::Image>(
        "/carla/" + role_name + "/rgb_right/image", 10,
    ...
    imageSubLeft = this->create_subscription<sensor_msgs::msg::Image>(
        "/carla/" + role_name + "/rgb_left/image", 10,
 ...
 private:
  ...
  void update_image(sensor_msgs::msg::Image &updatedImage,
