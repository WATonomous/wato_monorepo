...
class HUD (object):
    ...
    def render(self, display):
        ...
        if self.show_info:
            ...
            info_surface.fill(COLOR_ALUMINIUM_5)
...
class World(object):
    ...
    def __init__(self, name, args, timeout):
        ...
        # Hero actor (for HUD info display)
    ...
    def start(self, hud, input_control):
        ...
        # Map Mode - find existing hero vehicle for HUD info display
    ...
    def _find_hero_actor(self):
        """Finds an existing hero actor for HUD info display (does not spawn one)."""
    ...
    def update_hud_info(self, clock):
        """Updates the HUD info regarding simulation and ego vehicle status"""

        ego_vehicle_text = []
        if self.hero_actor is not None:
            ego_speed = self.hero_actor.get_velocity()
            ego_speed_text = 3.6 * math.sqrt(ego_speed.x ** 2 + ego_speed.y ** 2 + ego_speed.z ** 2)
            ...
            ego_vehicle_text = [
                'ID:                   %7d' % self.hero_actor.id,
                'Vehicle:       %14s' % get_actor_display_name(self.hero_actor, truncate=14),
                'Speed:               %3d km/h' % ego_speed_text,
                'Affected by:',
                '  Traffic Light: %12s' % affected_traffic_light_text,
                '  Speed Limit:       %3d km/h' % affected_speed_limit_text
            ]
        else:
            ego_vehicle_text = ['Not connected']

        self.server_fps = self.server_clock.get_fps()
        self.server_fps = 'inf' if self.server_fps == float('inf') else round(self.server_fps)
        # Truncate and wrap long map names to fit within the HUD panel
        map_name = self.town_map.name.split('/')[-1]  # Get just the map name, not the full path
        max_name_len = 20
        if len(map_name) > max_name_len:
            map_name = map_name[:max_name_len-1] + '\u2026'  # Truncate with ellipsis
        ...
        self._hud.add_info('EGO VEHICLE', ego_vehicle_text)
...
class InputControl(object):
    ...
    def _parse_mouse(self):
        """Parses mouse input for panning with left click and drag"""
        if pygame.mouse.get_pressed()[0]:
            if not self._dragging:
                # Start of drag - capture initial position
                self._dragging = True
                self.mouse_pos = pygame.mouse.get_pos()
            else:
                # Continue dragging - calculate delta and update offset
