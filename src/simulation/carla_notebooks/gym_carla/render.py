# For printing visualization to screen
...
class BirdeyeRenderSketch:
    def __init__(self, frame_width, frame_height, meters_to_pixels):
        self.birdeye_img = Image.new("RGB", (frame_width, frame_width))
        self.frame_width = frame_width
        self.frame_height = frame_height
        self.birdeye_draw = ImageDraw.Draw(self.birdeye_img)
        self.meters_to_pixels = meters_to_pixels
    def reset(self):
        self.birdeye_img = Image.new("RGB", (self.frame_width, self.frame_width))
        self.birdeye_draw = ImageDraw.Draw(self.birdeye_img)
    def draw_vehicle(self):
        """
        Draw vehicle in the bottom and center aligned horizontally 
        """
        half_width = self.meters_to_pixels*0.8635
        height = self.meters_to_pixels*3.821 
        self.birdeye_draw.rectangle([self.frame_width/2 - half_width, self.frame_height - height, self.frame_width/2 + half_width, self.frame_height], fill=COLOR_GRAY)
    def add_waypoints(self, waypoints, ego_x, ego_y, _ego_yaw, dot_size = 5):
        ego_yaw = _ego_yaw
        for waypoint in waypoints:
            y_local = (waypoint[0] - ego_x)*math.cos(-ego_yaw) - (waypoint[1] - ego_y)*math.sin(-ego_yaw)
            x_local = (waypoint[0] - ego_x)*math.sin(-ego_yaw) + (waypoint[1] - ego_y)*math.cos(-ego_yaw)
            x = self.frame_width/2 + x_local*self.meters_to_pixels
            y = self.frame_height - y_local*self.meters_to_pixels
            self.birdeye_draw.ellipse([(x - dot_size, y - dot_size),(x + dot_size, y + dot_size)], fill=COLOR_GREEN)
    def show_img(self):
        clear_output(wait=True)
        self.birdeye_img.show()
    def get_image(self):
        return(np.array(self.birdeye_img))
