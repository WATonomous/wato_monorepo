...
class CarlaEnv(gym.Env):
  ...
  def __init__(self, params):
    ...
    # parameters
    if('ego_vehicle_name' in params):
      self.ego_vehicle_name = params['ego_vehicle_name']
    else:
      self.ego_vehicle_name = 'ego'
    self.display_enabled = params['display']
    ...
    observation_space_dict = {
      ...
      # 'lidar': spaces.Box(low=0, high=255, shape=(self.obs_size, self.obs_size, 2), dtype=np.uint8),
      'birdeye': spaces.Box(low=0, high=255, shape=(self.obs_size, self.obs_size, 3), dtype=np.uint8),
      'state': spaces.Box(np.array([-2, -1, -5, 0]), np.array([2, 1, 30, 1]), dtype=np.float32)
      # 'speed_state': spaces.Box(low=-5, high=20, dtype=np.float32)
      }
    # if self.pixor:
    ...
    self.ego_bp.set_attribute("role_name", self.ego_vehicle_name)
    ...
    self.camera_trans = carla.Transform(carla.Location(x=0.8, z=2.0))
    ...
    self.camera_bp.set_attribute('sensor_tick', str(self.dt))
  ...
  def reset(self, seed=None, options=None):
    ...
    # self.birdeye_render = BirdeyeRender(self.obs_size, self.obs_size, 15)
    ...
    while True:
      ...
      if self._try_spawn_ego_vehicle_at(transform):
        ...
        time.sleep(0.1)
    # time.sleep(1)
    # act = carla.VehicleControl(steer=0.0, throttle=1.0)
  ...
  def step(self, action):
    ...
    if self.discrete:
      acc = self.discrete_act[0][action//self.n_acc]
    ...
    # if(self.long_speed < 0.5 and throttle > 0.1):
    #   init_seconds = default_timer()
    #   while default_timer() - init_seconds < 1.0:
    #     self.world.tick()
    # else:
    self.world.tick()
  ...
  def _get_obs(self):
    ...
    birdeye = pygame.surfarray.pixels3d(self.display)
    birdeye = birdeye[0:self.display_size, :, :]
    birdeye = display_to_rgb(birdeye, self.obs_size)

    # Convert the Pygame surface to a PIL image
    if(self.display_enabled):
      pil_image = Image.fromarray(birdeye.astype(np.uint8))
    ...
    # Display lidar image
    # z = np.zeros(lidar.shape[:-1] + (1,), dtype=lidar.dtype)
    # lidar_out = (lidar * 255).astype(dtype=np.uint8)
    ...
    # im = Image.fromarray(camera.astype(dtype=np.uint8))
    # im.show()
    ...
    state = np.array([lateral_dis, - delta_yaw, speed, self.vehicle_front]).astype(dtype=np.float32)

    # Create Image showing car and waypoints
    # self.birdeye_render.reset()
    # self.birdeye_render.draw_vehicle()
    # self.birdeye_render.add_waypoints(copy.deepcopy(self.waypoints), ego_x, ego_y, ego_yaw)
    # self.birdeye_render.show_img()
    
    
    # speed_state = np.array([speed]).astype(dtype=np.float32)
    # print(state)
    ...
    obs = {
      ...
      # 'birdeye': self.birdeye_render.get_image().astype(np.uint8),
  ...
  def _get_reward(self):
    ...
    r_speed = -abs(lspeed_lon - self.desired_speed)
    self.long_speed = lspeed_lon
    # cost for too fast
    # r_fast = 0
    ...
    # r = 200*r_collision + 1*lspeed_lon + 10*r_fast + r_speed + 1*r_out + r_steer*5 + 0.2*r_lat - 0.1
    r = 200*r_collision + 3*r_out + 10*r_speed
    # r = 200*r_collision + 200*r_speed
    return r
