...
- **`MpcControllerNode`** (`src/mpc_controller_node.cpp`) — lifecycle node and ROS plumbing.
  ...
  - **Failure handling:** if the solver fails or the reference is shorter than 2 points, it falls back to holding the previous steering and coasting (`current_speed + prev_accel·0.05`), throttled-warning logged.
