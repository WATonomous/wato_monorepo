...
#ifndef BEHAVIOUR__NODES__CONDITIONS__SAFE_PREDICTION_CONDITION_HPP_
#define BEHAVIOUR__NODES__CONDITIONS__SAFE_PREDICTION_CONDITION_HPP_
...
{
/**
   * @class SafePredictionCondition
   * @brief BT node to check if any dynamic objects' predicted trajectories will hit the car given a safe radius.
   */
class SafePredictionCondition : public BT::ConditionNode
{
public:
  SafePredictionCondition(const std::string & name, const BT::NodeConfig & config)
  ...
  BT::NodeStatus tick() override
  {
    ...
    for (const auto & obj : objects) {
      for (const auto & pred : obj.predictions) {
        for (const auto & pose : pred.poses) {
          double dx = pose.pose.position.x - current_point->x;
          double dy = pose.pose.position.y - current_point->y;
          double dist_sq = dx * dx + dy * dy;

          if (dist_sq < radius_sq) {
            // Predicted path enters the safe radius
            return BT::NodeStatus::FAILURE;
          }
        }
      }
...
#endif  // BEHAVIOUR__NODES__CONDITIONS__SAFE_PREDICTION_CONDITION_HPP_
