...
#ifndef BEHAVIOUR__NODES__COMMON__ACTIONS__GET_STOP_LINE_POSE_ACTION_HPP_
#define BEHAVIOUR__NODES__COMMON__ACTIONS__GET_STOP_LINE_POSE_ACTION_HPP_
...
namespace behaviour
{
     ...
     * @brief Extract a PoseStamped from a RegulatoryElement stop line (RefLine).
     *
     * Inputs:
     *  - reg_elem: lanelet_msgs::msg::RegulatoryElement::SharedPtr
     *
     * Outputs:
     ...
     *
     * Logic:
     *  - Uses the first ref_line in reg_elem->ref_lines.
     *  - Uses the midpoint of ref_line.points as position.
     *  - Sets yaw using a local tangent around the midpoint (mid-1 -> mid+1).
     *
     * Frame:
     *  - pose->header.frame_id is set to `input_frame` if provided, otherwise "map".
     *    (SpawnWallService will transform this pose to base_link.)
     */
...
#endif // BEHAVIOUR__NODES__COMMON__ACTIONS__GET_STOP_LINE_POSE_ACTION_HPP_
