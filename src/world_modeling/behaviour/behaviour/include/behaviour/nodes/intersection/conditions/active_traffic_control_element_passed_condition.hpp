...
namespace behaviour
{
     ...
     * @brief SUCCESS if ego has progressed past the lanelet that contains the active traffic-control element.
     *
     * MVP: route-index comparison only (with a small lanelet margin).
     *
     * Fail-safe: if any required inputs are missing or lanelet IDs are not found in the route index map,
     * return FAILURE (do not clear).
     */
    ...
    public:
        ...
        BT::NodeStatus tick() override
        {
            ...
            auto route_index_map =
                ports::tryGetPtr<std::unordered_map<int64_t, std::size_t>>(*this, "route_index_map");
            ...
            {
                std::cout << "[ActiveTrafficControlElementPassed] Missing active_traffic_control_lanelet_id"
                          << std::endl;
                return BT::NodeStatus::FAILURE;
            }
            ...
            if (it_active == route_index_map->end())
            {
                ...
                std::cout << "[ActiveTrafficControlElementPassed] active lanelet not in route_index_map: "
                          << active_id << std::endl;
            ...
            {
                std::cout << "[ActiveTrafficControlElementPassed] ego lanelet not in route_index_map: "
                          << ego_id << std::endl;
                return BT::NodeStatus::SUCCESS;
            }
            ...
            const std::size_t required =
                idx_active + static_cast<std::size_t>(std::max(0, lanelet_threshold));

            std::cout << "[ActiveTrafficControlElementPassed] idx_ego=" << idx_ego
                      << " idx_active=" << idx_active
                      << " required>=" << required
                      << " ego_lanelet=" << ego_id
                      << " active_lanelet=" << active_id
                      << std::endl;
