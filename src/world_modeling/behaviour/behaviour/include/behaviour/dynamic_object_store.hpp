...
//
// Assumptions:
// - `detection.results[0]` is the best class prediction when results exist.
...
private:
  ...
  // Returns the top-ranked class id from detection results, or empty view.
  static std::string_view getBestClassId(const ObjectT & o)
  {
    if (o.detection.results.empty()) return {};
  ...
  static std::vector<const ObjectT *> getObjects(const Snapshot & snap, const MapT & lanelet_map, const KeyT & key)
  {
    ...
    if (!snap.objects_snapshot_) return out;
  ...
  {
    if (!snap.objects_snapshot_) return nullptr;

    auto it = id_map.find(key);
    if (it == id_map.end()) return nullptr;

    const size_t idx = it->second;
    if (idx >= snap.objects_snapshot_->objects.size()) return nullptr;
