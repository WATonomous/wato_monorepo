  ...
  std::string & error)
{
  ...
  if (actual.size() != expected.size()) {
    error = "MTR " + binding_kind + " count mismatch: expected " + std::to_string(expected.size()) + ", got " +
  ...
  for (std::size_t i = 0; i < expected.size(); ++i) {
    ...
    if (!expected_names.emplace(expected_spec.name, true).second) {
      error = "duplicate expected MTR " + binding_kind + " binding name '" + expected_spec.name + "'";
      return false;
    }

    const auto actual_it = actual_by_name.find(expected_spec.name);
    if (actual_it == actual_by_name.end()) {
      error = binding_kind + "[" + std::to_string(i) + "] missing actual binding '" + expected_spec.name + "'";
      return false;
    }
    const auto & actual_spec = *actual_it->second;

    if (expected_spec.dtype != actual_spec.dtype) {
      error = binding_kind + " '" + expected_spec.name + "' dtype mismatch: expected '" + expected_spec.dtype +
              "', got '" + actual_spec.dtype + "'";
      return false;
    }
    ...
    for (std::size_t dimension = 0; dimension < expected_spec.shape.size(); ++dimension) {
      if (!dimensionMatches(expected_spec.shape[dimension], actual_spec.shape[dimension])) {
        error = binding_kind + " '" + expected_spec.name + "' shape mismatch at dimension " +
                std::to_string(dimension) + ": expected " + shapeToString(expected_spec.shape) + ", got " +
                shapeToString(actual_spec.shape);
        return false;
      }
    }
  }
...
MtrModelContract loadMtrModelContract(const std::string & metadata_path)
{
  ...
  const auto normalizeDtype = [&unquote](const std::string & value) {
    ...
    std::transform(
      dtype.begin(), dtype.end(), dtype.begin(), [](unsigned char character) { return std::tolower(character); });
  ...
  const auto parseShape = [&trim](const std::string & value, const std::size_t line_number) {
    ...
    if (text.size() < 2 || text.front() != '[' || text.back() != ']') {
      throw std::runtime_error(
        "invalid MTR metadata shape at line " + std::to_string(line_number) + ": expected [d0, d1, ...]");
    }
    ...
    while (std::getline(dimensions, token, ',')) {
      token = trim(token);
      if (token.empty()) {
        throw std::runtime_error("empty MTR metadata dimension at line " + std::to_string(line_number));
      }
      ...
      try {
        ...
        throw std::runtime_error(
          "invalid MTR metadata dimension '" + token + "' at line " + std::to_string(line_number));
      }
    ...
    if (shape.empty()) {
      throw std::runtime_error("empty MTR metadata shape at line " + std::to_string(line_number));
    }
    return shape;
  };
  ...
  const auto appendCurrent = [&]() {
    ...
    if (!has_name || !has_dtype || !has_shape) {
      throw std::runtime_error("incomplete MTR tensor entry in metadata file: " + metadata_path);
    }
    if (section == Section::Inputs) {
      if (std::any_of(contract.inputs.begin(), contract.inputs.end(), [&current](const MtrTensorSpec & spec) {
      ...
      if (std::any_of(contract.outputs.begin(), contract.outputs.end(), [&current](const MtrTensorSpec & spec) {
            return spec.name == current.name;
          }))
      {
      ...
      throw std::runtime_error("MTR tensor entry appears outside inputs/outputs in: " + metadata_path);
    }
  ...
  while (std::getline(file, line)) {
    ...
    if (line == "inputs:" || line == "outputs:") {
      ...
      if (line == "inputs:") {
        if (saw_inputs) {
          throw std::runtime_error("duplicate inputs section at line " + std::to_string(line_number));
        }
        saw_inputs = true;
        section = Section::Inputs;
      } else {
        if (saw_outputs) {
          throw std::runtime_error("duplicate outputs section at line " + std::to_string(line_number));
        }
        saw_outputs = true;
    ...
    if (section == Section::None) {
      throw std::runtime_error("unexpected MTR metadata content at line " + std::to_string(line_number) + ": " + line);
    }
    ...
    if (line.rfind("- ", 0) == 0) {
      ...
      throw std::runtime_error("expected '-' tensor entry at line " + std::to_string(line_number) + ": " + line);
    }

    const auto colon = line.find(':');
    if (colon == std::string::npos) {
      throw std::runtime_error("invalid MTR metadata field at line " + std::to_string(line_number));
    }
    const std::string key = trim(line.substr(0, colon));
    const std::string value = trim(line.substr(colon + 1));
    if (value.empty()) {
      throw std::runtime_error("empty MTR metadata field '" + key + "' at line " + std::to_string(line_number));
    }
    ...
    if (key == "name") {
      ...
      throw std::runtime_error("unknown MTR metadata field '" + key + "' at line " + std::to_string(line_number));
    }
  }
  appendCurrent();

  if (!saw_inputs || !saw_outputs || contract.inputs.empty() || contract.outputs.empty()) {
    throw std::runtime_error("MTR metadata must define non-empty inputs and outputs: " + metadata_path);
  }
  return contract;
}
