  ...
  for (int z = 0; z < NUM_ZONES; z++) {
    ...
    for (int r = 0; r < ZONE_RINGS[z]; r++) {
      ...
      for (int s = 0; s < ZONE_SECTORS[z]; s++) {
        index.sector_idx = s;
        index.patch_idx = patch_count;
        _patch_indices.push_back(index);
        pcl::PointCloud<pcl::PointXYZ> patch;
        ring.emplace_back(patch);
        _regionwise_ground.push_back(pcl::PointCloud<pcl::PointXYZ>>());
        _regionwise_nonground.push_back(pcl::PointCloud<pcl::PointXYZ>>());
...
void OccupancySegmentationCore::segment_ground() {
  tbb::parallel_for(tbb::blocked_range<int>(0, num_patches), [&](tbb::blocked_range<int> r) {
    for (int patch_num = r.begin(); patch_num < r.end(); r++) {
      Patch_Index &p_idx = patch_indices[patch_num];
      pcl::PointCloud<pcl::PointXYZ> &patch = czm[p_idx.zone_idx][p_idx.ring_idx][p_idx.sector_idx];
...
bool OccupancySegmentationCore::ground_likelihood_est(PCAFeature &feat, int concentric_idx){
...
bool OccupancySegmentationCore::point_z_cmp(pcl::PointXYZ a, pcl::PointXYZ b) { return a.z < b.z; }
