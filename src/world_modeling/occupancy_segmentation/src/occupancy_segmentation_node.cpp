...
: Node("occupancy_segmentation")
{
  ...
  // occupancy_segmentation_.update_position(pos_x.as_double(), pos_y.as_double(), pos_z.as_double());
  // occupancy_segmentation_.update_velocity(velocity.as_double());

  // param_cb_ = this->add_on_set_parameters_callback(
  //   std::bind(&OccupancySegmentationNode::parameters_callback, this, std::placeholders::_1));
}
...
void OccupancySegmentationNode::timer_callback()
{
  ...
  RCLCPP_INFO(this->get_logger(), "Publishing: %llu", msg.timestamp);
  data_pub_->publish(msg);
}

// rcl_interfaces::msg::SetParametersResult OccupancySegmentationNode::parameters_callback(const std::vector<rclcpp::Parameter> & parameters)
// {
...
//     {
//       occupancy_segmentation_.update_velocity(parameter.as_int());
