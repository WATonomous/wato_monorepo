...
OccupancySegmentationNode::OccupancySegmentationNode() : Node("occupancy_segmentation") {
  ...
  _nonground_publisher =
      this->create_publisher<sensor_msgs::msg::PointCloud2>("/nonground_points", 10);
}
...
void OccupancySegmentationNode::subscription_callback(
  ...
  // RCLCPP_INFO(this -> get_logger(), "Header incoming: %s", lidar_cloud ->
  ...
  RCLCPP_INFO(this->get_logger(), "Runtime for Patchwork: %i ms", duration);
