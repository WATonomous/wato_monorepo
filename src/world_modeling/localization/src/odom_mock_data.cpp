...
DummyPublisher::DummyPublisher() : Node("odom_mock_data") {
  ...
  timer_right_ = this->create_wall_timer(std::chrono::milliseconds(wheel_publisher_rate_),
                                         std::bind(&DummyPublisher::RandomLeftValues, this));
  timer_left_ = this->create_wall_timer(std::chrono::milliseconds(wheel_publisher_rate_),
                                        std::bind(&DummyPublisher::RandomRightValues, this));
  // publisher will fake turn values every 12 seconds
  timer_steering_ = this->create_wall_timer(std::chrono::milliseconds(wheel_turn_rate_),
                                            std::bind(&DummyPublisher::RandomSteeringValues, this));
}
...
void DummyPublisher::RandomLeftValues() {
  ...
  left_wheel_encoder =
      min_speed_value_ + ((double)rand() / RAND_MAX) * (max_speed_value_ - min_speed_value_);
...
void DummyPublisher::RandomSteeringValues() {
  ...
  steering_angle =
      min_turn_value_ + ((double)rand() / RAND_MAX) * (max_turn_value_ - min_turn_value_);
