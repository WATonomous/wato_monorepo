...
OdomMockData::OdomMockData() : Node("odom_mock_data") {
  ...
  publisher_right_wheel_ =
      this->create_publisher<std_msgs::msg::Float64>("right_motor_encoder", 10);
  publisher_steering_angle_ = this->create_publisher<std_msgs::msg::Float64>("steering_angle", 10);

  // publisher called every 1 second in meters
  timer_right_ = this->create_wall_timer(1000ms, std::bind(&OdomMockData::RandomLeftValues, this));
  timer_left_ = this->create_wall_timer(1000ms, std::bind(&OdomMockData::RandomRightValues, this));
