...
void ObjectsLayer::configure(rclcpp_lifecycle::LifecycleNode *node,
  ...
  node_->declare_parameter("layers." + layer_name_ + ".prediction_cost_decay",
                           0.3);
  node_->declare_parameter("layers." + layer_name_ + ".max_centroid_height_m",
                           100.0);

  bbox_inflation_m_ =
      node_->get_parameter("layers." + layer_name_ + ".bbox_inflation_m")
          .as_double();
  bbox_cost_decay_ =
      node_->get_parameter("layers." + layer_name_ + ".bbox_cost_decay")
          .as_double();
  prediction_cost_decay_ =
      node_->get_parameter("layers." + layer_name_ + ".prediction_cost_decay")
          .as_double();
  max_centroid_height_m_ =
      node_->get_parameter("layers." + layer_name_ + ".max_centroid_height_m")
          .as_double();
}

void ObjectsLayer::activate() {
  objects_sub_ =
      node_->create_subscription<world_model_msgs::msg::WorldObjectArray>(
          "world_objects", rclcpp::QoS(10),
          std::bind(&ObjectsLayer::objectsCallback, this,
                    std::placeholders::_1));
}

void ObjectsLayer::deactivate() { objects_sub_.reset(); }
...
void ObjectsLayer::update(
  ...
  for (const auto &obj : objects->objects) {
    ...
    if (bbox_inflation_m_ > 0.0) {
      int8_t inflated_cost =
          static_cast<int8_t>(std::max(1.0, 100.0 * bbox_cost_decay_));
      markBox(grid, pose_out.pose.position.x, pose_out.pose.position.y, obj_yaw,
              base_half_x + bbox_inflation_m_, base_half_y + bbox_inflation_m_,
              inflated_cost);
    }

    markBox(grid, pose_out.pose.position.x, pose_out.pose.position.y, obj_yaw,
            base_half_x, base_half_y, 100);
    ...
    for (const auto &prediction : obj.predictions) {
      for (const auto &ps : prediction.poses) {
        ...
        markBox(grid, pred_out.pose.position.x, pred_out.pose.position.y,
                yawFromQuat(pred_out.pose.orientation), pred_half_x,
                pred_half_y, pred_cost);
      }
    }
  }
}
