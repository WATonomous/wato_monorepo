  ...
  const TrackedObject & object, const Eigen::Vector2d & point) const
{
  ...
  const auto pedestrian_to_entry_point_orientation = autoware::universe_utils::createQuaternionFromYaw(
  ...
  const PosePath & ref_path) const
{
  ...
  for (size_t i = 0; i < predicted_path.path.size(); ++i) {
    ...
    // Converted Pose
    auto predicted_pose = autoware::universe_utils::calcOffsetPose(ref_pose, 0.0, frenet_point.d, 0.0);
  ...
  const double speed_limit) const
{
  ...
  const size_t nearest_segment_idx = autoware::motion_utils::findNearestSegmentIndex(ref_path, obj_point);
  const double l =
    autoware::motion_utils::calcLongitudinalOffsetToSegment(ref_path, nearest_segment_idx, obj_point);
  ...
  frenet_point.s = autoware::motion_utils::calcSignedArcLength(ref_path, 0, nearest_segment_idx) + l;
