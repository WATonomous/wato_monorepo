...
#include "motion_forecasting/motion_forecasting_node.hpp"
...
lanelet::ConstLanelets getLanelets(const motion_forecasting::LaneletsData & data)
{
...
: Node("motion_forecasting", node_options), debug_accumulated_time_(0.0)
{
  if (!google::IsGoogleLoggingInitialized()) {
    google::InitGoogleLogging("motion_forecasting_node");
  ...
  min_velocity_for_motion_forecasting_ =
    declare_parameter<double>("min_velocity_for_motion_forecasting");
  ...
  processing_time_publisher_ =
    std::make_unique<autoware_universe_utils::DebugPublisher>(this, "motion_forecasting");

  published_time_publisher_ = std::make_unique<autoware_universe_utils::PublishedTimePublisher>(this);
  set_param_res_ = this->add_on_set_parameters_callback(
    std::bind(&MapBasedPredictionNode::onParam, this, std::placeholders::_1));

  stop_watch_ptr_ = std::make_unique<autoware_universe_utils::StopWatch<std::chrono::milliseconds>>();
  ...
  for (const auto & object : in_objects->objects) {
    ...
    switch (label) {
      ...
      case ObjectClassification::TRUCK: {
        ...
        if (std::fabs(abs_obj_speed) < min_velocity_for_motion_forecasting_) {
  ...
  switch (object.kinematics.orientation_availability) {
    case autoware_perception_msgs::msg::DetectedObjectKinematics::SIGN_UNKNOWN: {
      ...
      object.kinematics.pose_with_covariance.pose.orientation =
        autoware_universe_utils::createQuaternionFromYaw(autoware_universe_utils::pi + original_yaw);
      break;
    }
    default: {
      const auto updated_object_yaw =
        autoware_universe_utils::calcAzimuthAngle(object_pose.position, future_object_pose.position);
  ...
  const LaneletsData & current_lanelets_data)
{
  ...
  const double object_yaw = tf2::getYaw(object.kinematics.pose_with_covariance.pose.orientation);
  single_object_data.pose.orientation = autoware_universe_utils::createQuaternionFromYaw(object_yaw);
  ...
  const lanelet::Id & signal_id)
{
  ...
  const auto key = std::make_pair(autoware_universe_utils::toHexString(object.object_id), signal_id);
  if (
...
RCLCPP_COMPONENTS_REGISTER_NODE(motion_forecasting::MapBasedPredictionNode)
