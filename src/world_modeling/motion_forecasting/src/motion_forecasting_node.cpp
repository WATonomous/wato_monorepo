...
: Node("motion_forecasting", node_options), debug_accumulated_time_(0.0)
{
  ...
  processing_time_publisher_ =
    std::make_unique<autoware::universe_utils::DebugPublisher>(this, "motion_forecasting");
  ...
  switch (object.kinematics.orientation_availability) {
    case autoware_perception_msgs::msg::DetectedObjectKinematics::SIGN_UNKNOWN: {
      ...
      object.kinematics.pose_with_covariance.pose.orientation =
        autoware::universe_utils::createQuaternionFromYaw(autoware::universe_utils::pi + original_yaw);
      break;
    }
    default: {
      const auto updated_object_yaw =
        autoware::universe_utils::calcAzimuthAngle(object_pose.position, future_object_pose.position);
  ...
  const LaneletsData & current_lanelets_data)
{
  ...
  const double object_yaw = tf2::getYaw(object.kinematics.pose_with_covariance.pose.orientation);
  single_object_data.pose.orientation = autoware::universe_utils::createQuaternionFromYaw(object_yaw);
  ...
  const lanelet::Id & signal_id)
{
  ...
  const auto key = std::make_pair(autoware::universe_utils::toHexString(object.object_id), signal_id);
  if (
