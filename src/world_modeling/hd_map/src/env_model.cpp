...
public:
  EnvModel() : Node("env_model") {
   // create a Lanelet2 map
    lanelet::LaneletMapPtr map = std::make_shared<lanelet::LaneletMap>();

    // create 5 random lanelets
    for (int i = 0; i < 5; ++i) {
        // create some random points
        lanelet::Point3d point1(i*20, 0, 0);
        lanelet::Point3d point2(i*20 + 10, 0, 0);
        lanelet::Point3d point3(i*20 + 10, 10, 0);
        lanelet::Point3d point4(i*20, 10, 0);

        // create a polygon from the points
        lanelet::Polygon3d polygon({point1, point2, point3, point4});

        // create a lanelet from the polygon
        lanelet::LineString3d centerline({point1, point2, point3, point4});
        lanelet::Lanelet lanelet(lanelet::utils::getId(), centerline, {polygon});
