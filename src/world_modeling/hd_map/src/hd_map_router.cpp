...
bool HDMapRouter::set_lanelet(const lanelet::LaneletMapPtr& lanelet_ptr) {
  ...
  lanelet::traffic_rules::TrafficRulesPtr traffic_rules{
      lanelet::traffic_rules::TrafficRulesFactory::instance().create(
  ...
  if (errors.empty() != true) {
    ...
    for (auto error : errors) {
      RCLCPP_INFO(rclcpp::get_logger("hd_map_router"), "Routing Graph Build Error : %s",
                  error.c_str());
    }
    return false;
  }
...
lanelet::LaneletMapPtr HDMapRouter::get_lanelet() { return this->lanelet_ptr_; }
...
lanelet::ConstLanelet HDMapRouter::get_nearest_lanelet_to_xy(float x, float y, float width_x,
                                                             float height_y) {
...
lanelet::ConstLanelet HDMapRouter::get_nearest_lanelet_to_xyz(float x_center, float y_center,
                                                              float z_center) {
...
lanelet::Optional<lanelet::routing::LaneletPath> HDMapRouter::route(lanelet::GPSPoint from_point,
...
void HDMapRouter::process_traffic_sign_msg(
...
void HDMapRouter::process_pedestrian_msg(
    const vision_msgs::msg::Detection3DArray::SharedPtr pedestrian_msg_ptr) {
  ...
  for (const auto& reg_elem : lanelet_ptr_->regulatoryElementLayer) {
    ...
    if (traffic_light_elem && traffic_light_elem->getId() == traffic_light_id) {
      ...
      RCLCPP_INFO(rclcpp::get_logger("hd_map_router"),
                  ...
                  traffic_light_id, bbox.center.position.x, bbox.center.position.y,
...
void HDMapRouter::update_pedestrian(
  ...
  if (pedestrian_class != "PEDESTRIAN") {
    RCLCPP_ERROR(rclcpp::get_logger("hd_map_router"),
                 "Received non-pedestrian message in update_pedestrian function!");
    return;
  }
  ...
  RCLCPP_ERROR(rclcpp::get_logger("hd_map_router"), "Pedestrian with ID %lu not found for update.",
               pedestrian_id);
}
  ...
  for (const auto& reg_elem : lanelet_ptr_->regulatoryElementLayer) {
    ...
    if (traffic_light_elem && traffic_light_elem->getId() == traffic_light_id) {
      for (auto& lanelet : lanelet_ptr_->laneletLayer) {
        lanelet.removeRegulatoryElement(
            traffic_light_elem);  // true if removed, false if not in reg elems
      }
      ...
      RCLCPP_INFO(rclcpp::get_logger("hd_map_router"),
                  "Removed traffic light from the lanelet map: ID = %lu", traffic_light_id);
      return;
    }
  }

  RCLCPP_ERROR(rclcpp::get_logger("hd_map_router"),
               "Traffic light with ID %lu not found for removal.", traffic_light_id);
}
...
void HDMapRouter::remove_pedestrian(uint64_t pedestrian_id) {
  for (const auto& reg_elem : lanelet_ptr_->regulatoryElementLayer) {
    ...
    if (pedestrian_elem && pedestrian_elem->getId() == pedestrian_id) {
      ...
      RCLCPP_INFO(rclcpp::get_logger("hd_map_router"),
                  "Removed pedestrian from the lanelet map: ID = %lu", pedestrian_id);
      return;
    }
  }

  RCLCPP_ERROR(rclcpp::get_logger("hd_map_router"), "Pedestrian with ID %lu not found for removal.",
               pedestrian_id);
}
