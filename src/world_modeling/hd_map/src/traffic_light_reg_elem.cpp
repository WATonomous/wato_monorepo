...
std::shared_ptr<TrafficLightRegElem> TrafficLightRegElem::make(const lanelet::BoundingBox3d& bbox,
                                                               const TrafficLightState& state,
                                                               uint64_t id) {
...
TrafficLightRegElem::TrafficLightRegElem(const lanelet::RegulatoryElementDataPtr& data,
                                         const TrafficLightState& state, uint64_t id)
    : lanelet::RegulatoryElement(data), state{state}, id{id} {}

void TrafficLightRegElem::updateTrafficLight(const lanelet::BoundingBox3d& bbox,
                                             const TrafficLightState& state) {
  auto traffic_light = utils::boundingBox3dToPolygon3d(bbox);
  parameters()[lanelet::RoleName::Refers].clear();
  parameters()[lanelet::RoleName::Refers].emplace_back(
      traffic_light);  // Update the traffic light polygon

  this->state = state;
}

uint64_t TrafficLightRegElem::getId() const { return id; }

TrafficLightState TrafficLightRegElem::getState() const { return state; }
