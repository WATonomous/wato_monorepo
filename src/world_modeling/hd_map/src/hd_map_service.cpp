...
HDMapService::HDMapService() : Node("hd_map_service") {
  ...
  this->declare_parameter<std::string>("desired_lane_output_topic",
                                       std::string("hd_map_desired_lane"));
  this->declare_parameter<std::string>("current_lane_output_topic",
                                       std::string("hd_map_current_lane"));
  ...
  std::string desired_lane_output_topic =
      this->get_parameter("desired_lane_output_topic").as_string();
  std::string current_lane_output_topic =
      this->get_parameter("current_lane_output_topic").as_string();

  std::string traffic_light_input_topic =
      this->get_parameter("traffic_light_input_topic").as_string();
  std::string traffic_sign_input_topic =
      this->get_parameter("traffic_sign_input_topic").as_string();
  ...
  behaviour_tree_info_service = this->create_service<world_modeling_msgs::srv::BehaviourTreeInfo>(
      "behaviour_tree_info", std::bind(&HDMapService::behaviour_tree_info_callback, this,
  ...
  hd_map_start_publisher_ =
      this->create_publisher<visualization_msgs::msg::MarkerArray>(start_output_topic, 20);
  hd_map_end_publisher_ =
      this->create_publisher<visualization_msgs::msg::MarkerArray>(end_output_topic, 20);
  ...
  hd_map_traffic_sign_subscriber_ = this->create_subscription<vision_msgs::msg::Detection3D>(
      traffic_sign_input_topic, 20,
      std::bind(&HDMapService::hd_map_traffic_sign_callback, this, std::placeholders::_1));
  hd_map_pedestrian_subscriber_ = this->create_subscription<vision_msgs::msg::Detection3DArray>(
      pedestrian_input_topic, 20,
      std::bind(&HDMapService::hd_map_pedestrian_callback, this, std::placeholders::_1));
  ...
  query_point_subscriber_ = this->create_subscription<geometry_msgs::msg::PointStamped>(
      query_point_input_topic, 20,
      std::bind(&HDMapService::get_desired_lane, this, std::placeholders::_1));

  hd_map_visualization_timer_ = this->create_wall_timer(
      std::chrono::milliseconds(5000), std::bind(&HDMapService::publish_hd_map_marker, this));
}

void HDMapService::hd_map_traffic_light_callback(
...
void HDMapService::hd_map_traffic_sign_callback(
    vision_msgs::msg::Detection3D::SharedPtr traffic_sign_msg) {
  router_->process_traffic_sign_msg(traffic_sign_msg);
}

void HDMapService::hd_map_pedestrian_callback(
    vision_msgs::msg::Detection3DArray::SharedPtr pedestrian_msg) {
...
void HDMapService::point_callback(geometry_msgs::msg::PointStamped::SharedPtr msg) {
  ...
  auto marker1 =
      world_modeling::hd_map::laneletAsMarkerArray(lanelet1, &id1, false, true, color, color);
  hd_map_start_publisher_->publish(marker1);
  int id2 = 0;
  auto marker2 =
      world_modeling::hd_map::laneletAsMarkerArray(lanelet2, &id2, false, true, color, color);
...
void HDMapService::get_desired_lane(geometry_msgs::msg::PointStamped::SharedPtr msg) {
  ...
  auto pt = router_->project_point3d_to_gps(
      lanelet::BasicPoint3d{msg->point.x, msg->point.y, msg->point.z});
  ...
  if (lanelet_path) {
    ...
    if (it != lanelet_path->end()) {
      ...
      if (lanelet_path->getRemainingLane(it).size() > 1) {
        ...
        current_marker = world_modeling::hd_map::laneletAsMarkerArray(*it, &id, false, true, color,
                                                                      color, .3, .4);
        desired_marker = world_modeling::hd_map::laneletAsMarkerArray(*it, &id, false, true, color,
                                                                      color, .3, .4);
...
world_modeling_msgs::msg::Lanelet HDMapService::convert_lanelet_to_msg(
