...
WorldObjectsMarkersNode::WorldObjectsMarkersNode()
  ...
  prediction_line_width_ =
      this->get_parameter("prediction_line_width").as_double();
  lanelet_boundary_line_width_ =
      this->get_parameter("lanelet_boundary_line_width").as_double();
  lanelet_centerline_line_width_ =
      this->get_parameter("lanelet_centerline_line_width").as_double();
  lanelet_boundary_color_r_ =
      this->get_parameter("lanelet_boundary_color_r").as_double();
  lanelet_boundary_color_g_ =
      this->get_parameter("lanelet_boundary_color_g").as_double();
  lanelet_boundary_color_b_ =
      this->get_parameter("lanelet_boundary_color_b").as_double();
  lanelet_boundary_alpha_ =
      this->get_parameter("lanelet_boundary_alpha").as_double();
  lanelet_centerline_color_r_ =
      this->get_parameter("lanelet_centerline_color_r").as_double();
  lanelet_centerline_color_g_ =
      this->get_parameter("lanelet_centerline_color_g").as_double();
  lanelet_centerline_color_b_ =
      this->get_parameter("lanelet_centerline_color_b").as_double();
  lanelet_centerline_alpha_ =
      this->get_parameter("lanelet_centerline_alpha").as_double();
  hypothesis_idx_ =
      this->get_parameter("entity_class_hypothesis_index").as_int();

  publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
      "markers", 10);
  subscription_ =
      this->create_subscription<world_model_msgs::msg::WorldObjectArray>(
          "world_objects", 10,
          std::bind(&WorldObjectsMarkersNode::worldObjectsCallback, this,
                    std::placeholders::_1));
...
WorldObjectsMarkersNode::getColorForClassId(const std::string &class_id) const {
  if (class_id == "car" || class_id == "Car" || class_id == "CAR" ||
      class_id == "vehicle" || class_id == "truck") {
    return lanelet_markers::makeColor(0.2f, 0.4f, 1.0f,
                                      static_cast<float>(box_alpha_));
  } else if (class_id == "human" || class_id == "Human" ||
             class_id == "HUMAN" || class_id == "pedestrian" ||
             class_id == "Pedestrian" || class_id == "PEDESTRIAN" ||
             class_id == "person") {
    return lanelet_markers::makeColor(1.0f, 1.0f, 0.0f,
                                      static_cast<float>(box_alpha_));
  } else if (class_id == "bicycle" || class_id == "Bicycle" ||
             class_id == "BICYCLE" || class_id == "cyclist") {
    return lanelet_markers::makeColor(0.0f, 1.0f, 1.0f,
                                      static_cast<float>(box_alpha_));
  } else if (class_id == "motorcycle" || class_id == "Motorcycle" ||
             class_id == "MOTORCYCLE" || class_id == "motorbike") {
    return lanelet_markers::makeColor(1.0f, 0.0f, 1.0f,
                                      static_cast<float>(box_alpha_));
  } else if (class_id == "traffic_light" || class_id == "TrafficLight" ||
             class_id == "TRAFFIC_LIGHT") {
    return lanelet_markers::makeColor(1.0f, 1.0f, 1.0f,
                                      static_cast<float>(box_alpha_));
  }
  // UNKNOWN / default
  return lanelet_markers::makeColor(0.6f, 0.6f, 0.6f,
                                    static_cast<float>(box_alpha_));
}

std_msgs::msg::ColorRGBA
WorldObjectsMarkersNode::getColorForConfidence(double confidence,
  ...
  return lanelet_markers::makeColor(static_cast<float>(1.0 - clamped_conf),
                                    static_cast<float>(clamped_conf), 0.0f,
                                    alpha);
}
...
void WorldObjectsMarkersNode::worldObjectsCallback(
  ...
  for (const auto &world_obj : msg->objects) {
    ...
    if (!det.results.empty() &&
        static_cast<size_t>(hypothesis_idx_) < det.results.size()) {
    ...
    auto box_marker = lanelet_markers::createCubeMarker(
        "wo_boxes", marker_id++, frame_id, det.bbox.center, det.bbox.size,
        color);
    ...
    if (world_obj.history.size() >= 2) {
      ...
      auto history_marker = lanelet_markers::createLineStripMarker(
          "wo_history", marker_id++, frame_id, history_points, history_color,
          history_line_width_);
      ...
      for (const auto &lanelet : world_obj.lanelet_ahead.lanelets) {
        ...
        if (lanelet.left_boundary.size() >= 2) {
          auto left_marker = lanelet_markers::createLineStripMarker(
              "wo_lanelet_ahead", marker_id++, frame_id, lanelet.left_boundary,
              lanelet_color, lanelet_boundary_line_width_);
        ...
        if (lanelet.right_boundary.size() >= 2) {
          auto right_marker = lanelet_markers::createLineStripMarker(
              "wo_lanelet_ahead", marker_id++, frame_id, lanelet.right_boundary,
              lanelet_color, lanelet_boundary_line_width_);
        ...
        if (lanelet.centerline.size() >= 2) {
          auto cl_marker = lanelet_markers::createLineStripMarker(
              "wo_lanelet_ahead", marker_id++, frame_id, lanelet.centerline,
              centerline_color, lanelet_centerline_line_width_);
...
int main(int argc, char **argv) {
  ...
  rclcpp::spin(
      std::make_shared<world_model_markers::WorldObjectsMarkersNode>());
  rclcpp::shutdown();
  return 0;
}
