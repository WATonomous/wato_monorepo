...
"""Convert a lanelet2 OSM map from local cartesian (local_x, local_y, ele)
to WGS84 (lat, lon, ele) using the UTM origin stored in an eidos .map file.

Usage:
    python3 lanelet_local_to_wgs84.py \\
        --map /path/to/wrestrc.map \\
        --input /path/to/local.osm \\
        --output /path/to/wgs84.osm

The .map file is a SQLite database containing a global_data entry for
'gps_factor/utm_to_map' — 5 packed doubles: [offset_x, offset_y, offset_z,
encoded_zone, yaw].
...
def load_gps_config_from_map(map_path: str):
    """Read gps_factor/utm_to_map from an eidos .map SQLite file.
    ...
    if row is None:
        raise ValueError(
            f"No 'gps_factor/utm_to_map' entry in {map_path}. "
            "Was the map built with GPS enabled?"
        )

    data = row[0]
    if len(data) != 5 * 8:
        raise ValueError(f"utm_to_map blob has {len(data)} bytes, expected 40")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        "using an eidos .map file's GPS offset."
    )
    parser.add_argument(
        "--map",
        required=True,
        help="Path to eidos .map file (SQLite with gps_factor/utm_to_map)",
    )
    parser.add_argument(
        "--input",
        required=True,
        help="Input OSM file with local_x/local_y coordinates",
    )
    parser.add_argument(
        "--output",
        required=True,
        help="Output OSM file with lat/lon coordinates",
    )
    args = parser.parse_args()

    try:
        offset_x, offset_y, offset_z, utm_zone, is_north, yaw = (
            load_gps_config_from_map(args.map)
        )
