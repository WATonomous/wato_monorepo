...
ImuIntegrationFactor::getFactors(int state_index,
  ...
  if (state_index == 0) {
    ...
    double dt = imu_integrator_opt_->deltaTij();

    if (dt > 5.0) {
      // Preintegration spans too long (e.g. bag jump / queue backlog).
      // The accumulated drift would corrupt the graph, so skip this factor
      // and reset to the current pose.
      RCLCPP_WARN(node_->get_logger(),
          "[%s] Skipping ImuFactor %d->%d: dt=%.3f s exceeds 5.0 s limit, resetting",
          name_.c_str(), state_index - 1, state_index, dt);
      imu_integrator_opt_->resetIntegrationAndSetBias(prev_bias_);

      // Still need to provide V and B initial values for this state
      result.values.insert(V(state_index), prev_vel_);
      result.values.insert(B(state_index), prev_bias_);

      // Add weak priors so these variables aren't unconstrained
      result.factors.push_back(
          gtsam::make_shared<gtsam::PriorFactor<gtsam::Vector3>>(
              V(state_index), prev_vel_, prior_vel_noise_));
      result.factors.push_back(
          gtsam::make_shared<gtsam::PriorFactor<gtsam::imuBias::ConstantBias>>(
              B(state_index), prev_bias_, prior_bias_noise_));
