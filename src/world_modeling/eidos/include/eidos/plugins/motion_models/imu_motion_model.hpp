...
private:
  ...
  // ---- IMU buffer for generateMotionModel() ----
  ...
  // ---- State ----
  // Reference state for real-time odometry prediction (set once at init, never updated)
  gtsam::NavState odom_reference_state_;
  gtsam::imuBias::ConstantBias odom_reference_bias_;

  // State for factor generation (updated after each generateMotionModel call)
