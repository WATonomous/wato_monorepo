...
/**
 * @brief Helper to call a Trigger service synchronously
 */
std::pair<bool, std::string> call_trigger_service(
  rclcpp::Node::SharedPtr node, const std::string & service_name, std::chrono::seconds timeout = 10s)
{
  ...
  wato::test::MultiThreadedTestFixture, "LifecycleManager startup service", "[lifecycle_manager][startup]")
{
  ...
  SECTION("Startup transitions nodes to active state")
  {
    ...
    auto [success, message] = call_trigger_service(client_node, "/lifecycle_manager/startup");
  ...
  wato::test::MultiThreadedTestFixture, "LifecycleManager shutdown service", "[lifecycle_manager][shutdown]")
{
  ...
  SECTION("Shutdown transitions active nodes to unconfigured state")
  {
    ...
    auto [startup_success, startup_msg] = call_trigger_service(client_node, "/lifecycle_manager/startup");
    ...
    // Now call shutdown
    auto [shutdown_success, shutdown_msg] = call_trigger_service(client_node, "/lifecycle_manager/shutdown");
  ...
  wato::test::MultiThreadedTestFixture, "LifecycleManager is_active service", "[lifecycle_manager][is_active]")
{
  ...
  {
    auto [success, message] = call_trigger_service(client_node, "/lifecycle_manager/is_active");
  ...
  "[lifecycle_manager][pause][resume]")
{
  ...
  SECTION("Pause deactivates nodes without cleanup")
  {
    ...
    // Pause
    auto [pause_success, pause_msg] = call_trigger_service(client_node, "/lifecycle_manager/pause");
  ...
  SECTION("Resume activates paused nodes")
  {
    ...
    // Resume
    auto [resume_success, resume_msg] = call_trigger_service(client_node, "/lifecycle_manager/resume");
...
TEST_CASE_METHOD(wato::test::MultiThreadedTestFixture, "LifecycleManager reset service", "[lifecycle_manager][reset]")
{
  ...
  SECTION("Reset performs shutdown then startup")
  {
    ...
    // Reset
    auto [reset_success, reset_msg] = call_trigger_service(client_node, "/lifecycle_manager/reset");
  ...
  "[lifecycle_manager][idempotency]")
{
  ...
  {
    // First startup
    auto [success1, msg1] = call_trigger_service(client_node, "/lifecycle_manager/startup");
    ...
    auto [success2, msg2] = call_trigger_service(client_node, "/lifecycle_manager/startup");
  ...
  SECTION("Calling shutdown twice succeeds")
  {
    ...
    // First shutdown
    auto [success1, msg1] = call_trigger_service(client_node, "/lifecycle_manager/shutdown");
    REQUIRE(success1 == true);
    ...
    auto [success2, msg2] = call_trigger_service(client_node, "/lifecycle_manager/shutdown");
    REQUIRE(success2 == true);
    REQUIRE(mock_node->get_current_state() == State::PRIMARY_STATE_UNCONFIGURED);
  ...
  SECTION("Calling pause when not active has no effect on unconfigured node")
  {
    ...
    auto [success, message] = call_trigger_service(client_node, "/lifecycle_manager/pause");
  ...
  SECTION("Calling resume when not paused")
  {
    ...
    auto [success, message] = call_trigger_service(client_node, "/lifecycle_manager/resume");
    REQUIRE(success == true);
    REQUIRE(mock_node->get_current_state() == State::PRIMARY_STATE_ACTIVE);
