...
def main(args = None):
    ...
    # node.declare_parameter("config_path", "/home/bolty/ament_ws/src/depth_estimation/config/config.yaml")
    # node.declare_parameter('publish_frequency', 20)
    ...
    depth_estimation_node = DepthAnything()

    try:
        while rclpy.ok():
            # try:
            #     depth_estimation_node.depth_pcl_publisher.publish(1 / 0.8) # Messages are published every 1.25 seconds
            # except Exception as e:
            #     depth_estimation_node.get_logger().error(f"Error: Exception caught while processing a frame: {e}")
            try:
                # rclpy.spin_once(depth_estimation_node, timeout_sec=1.0 / 0.8) # Node has 1.25 seconds to process and publish messages
                rclpy.spin_once(depth_estimation_node, timeout_sec=60)
            except ROSInterruptException:
                pass
    except ROSInterruptException:
        depth_estimation_node.get_logger().info("Shutting down")
