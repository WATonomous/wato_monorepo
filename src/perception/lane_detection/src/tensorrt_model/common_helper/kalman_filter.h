...
class KalmanFilter {
 public:
  KalmanFilter() : sigma_true(1.0), sigma_observe(1.0) {}

  ~KalmanFilter() {}

  void Initialize(const SimpleMatrix& _F, const SimpleMatrix& _Q, const SimpleMatrix& _H,
                  const SimpleMatrix& _R, const SimpleMatrix& _X, const SimpleMatrix& _P) {
    F = _F;
    Q = _Q;
    H = _H;
    R = _R;
