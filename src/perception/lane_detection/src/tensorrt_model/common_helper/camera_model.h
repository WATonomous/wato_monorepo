...
class CameraModel {
   ...
   *
   * 注意2: 座標系は右手系。X+ = 右、Y+ = 下、Z+ = 奥  (例.
   *カメラから見て物体が上にある場合、Ycは負値)
   ***/
 ...
 public:
  ...
  /* float, 3 x 1, (tx, ty, tz): horizontal, vertical, depth (Camera location: Ow - Oc in camera
   * coordinate) */
  cv::Mat tvec;
 ...
 public:
  ...
  void SetIntrinsic(int32_t width, int32_t height, float focal_length) {
    this->width = width;
    this->height = height;
    this->K = (cv::Mat_<float>(3, 3) << focal_length, 0, width / 2.f, 0, focal_length, height / 2.f,
               0, 0, 1);
  ...
  void UpdateNewCameraMatrix() {
    if (!this->K.empty() && !this->dist_coeff.empty()) {
      this->K_new = cv::getOptimalNewCameraMatrix(this->K, this->dist_coeff,
                                                  cv::Size(this->width, this->height), 0.0);
    }
  }

  void SetExtrinsic(const std::array<float, 3>& rvec_deg, const std::array<float, 3>& tvec,
                    bool is_t_on_world = true) {
    this->rvec =
        (cv::Mat_<float>(3, 1) << Deg2Rad(rvec_deg[0]), Deg2Rad(rvec_deg[1]), Deg2Rad(rvec_deg[2]));
  ...
  void GetExtrinsic(std::array<float, 3>& rvec_deg, std::array<float, 3>& tvec,
                    bool is_t_on_world = true) {
    rvec_deg = {Rad2Deg(this->rvec.at<float>(0)), Rad2Deg(this->rvec.at<float>(1)),
                Rad2Deg(this->rvec.at<float>(2))};
  ...
  void ConvertWorld2Image(const std::vector<cv::Point3f>& object_point_list,
    ...
    cv::Mat Rt = (cv::Mat_<float>(3, 4) << R.at<float>(0), R.at<float>(1), R.at<float>(2),
                  this->tx(), R.at<float>(3), R.at<float>(4), R.at<float>(5), this->ty(),
...
#else
    cv::projectPoints(object_point_list, this->rvec, this->tvec, this->K, this->dist_coeff,
                      image_point_list);
#endif
  }
...
#pragma omp parallel for
    ...
    for (int32_t i = 0; i < object_point_in_world_list.size(); i++) {
      ...
      cv::Mat Mw = (cv::Mat_<float>(4, 1) << object_point_in_world.x, object_point_in_world.y,
                    object_point_in_world.z, 1);
      cv::Mat Mc = Rt * Mw;
...
#pragma omp parallel for
    ...
    for (int32_t i = 0; i < object_point_in_camera_list.size(); i++) {
      ...
      cv::Mat Mc = (cv::Mat_<float>(3, 1) << object_point_in_camera.x, object_point_in_camera.y,
                    object_point_in_camera.z);
  ...
  void ConvertImage2Camera(std::vector<cv::Point2f>& image_point_list,
                           const std::vector<float>& z_list,
                           std::vector<cv::Point3f>& object_point_list) {
    /*** Image -> Mc ***/
    ...
    std::vector<cv::Point2f> image_point_undistort;
    if (this->dist_coeff.empty() || this->dist_coeff.at<float>(0) == 0) {
      image_point_undistort =
          image_point_list; /* todo: redundant copy. I do this just because to make code simple */
    } else {
      cv::undistortPoints(image_point_list, image_point_undistort, this->K, this->dist_coeff,
                          this->K); /* don't use K_new */
    }
...
#pragma omp parallel for
  ...
  void ConvertImage2World(std::vector<cv::Point2f>& image_point_list,
                          const std::vector<float>& z_list,
                          std::vector<cv::Point3f>& object_point_list) {
    /*** Image -> Mw ***/
    std::vector<cv::Point3f> object_point_in_camera_list;
  ...
  static void RotateObject(float x_deg, float y_deg, float z_deg,
                           std::vector<cv::Point3f>& object_point_list) {
