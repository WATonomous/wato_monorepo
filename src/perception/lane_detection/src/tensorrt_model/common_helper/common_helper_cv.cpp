...
void CommonHelper::DrawText(cv::Mat& mat, const std::string& text, cv::Point pos, double font_scale,
  ...
  if (is_text_on_rect) {
    cv::rectangle(mat, pos + cv::Point(0, baseline),
                  pos + cv::Point(textSize.width, -textSize.height), color_back, -1);
...
void CommonHelper::CropResizeCvt(const cv::Mat& org, cv::Mat& dst, int32_t& crop_x, int32_t& crop_y,
                                 int32_t& crop_w, int32_t& crop_h, bool is_rgb, int32_t crop_type,
                                 bool resize_by_linear) {
...
std::string CommonHelper::CreateGStreamerPipeline(int capture_width, int capture_height,
...
bool CommonHelper::FindSourceImage(const std::string& input_name, cv::VideoCapture& cap,
                                   int32_t width, int32_t height) {
  if (input_name.find(".mp4") != std::string::npos ||
      input_name.find(".avi") != std::string::npos ||
  ...
  } else if (input_name.find(".jpg") != std::string::npos ||
             input_name.find(".png") != std::string::npos ||
...
cv::Mat CommonHelper::CombineMat1to3(const cv::Mat& mat0, const cv::Mat& mat1,
                                     const cv::Mat& mat2) {
...
cv::Mat CommonHelper::CombineMat1to3(int32_t rows, int32_t cols, float* data0, float* data1,
                                     float* data2) {
