...
#endif
#if defined(INFERENCE_HELPER_ENABLE_TFLITE) ||                  \
    defined(INFERENCE_HELPER_ENABLE_TFLITE_DELEGATE_XNNPACK) || \
    defined(INFERENCE_HELPER_ENABLE_TFLITE_DELEGATE_GPU) ||     \
    defined(INFERENCE_HELPER_ENABLE_TFLITE_DELEGATE_EDGETPU)
...
#endif
#if defined(INFERENCE_HELPER_ENABLE_ONNX_RUNTIME) || \
    defined(INFERENCE_HELPER_ENABLE_ONNX_RUNTIME_CUDA)
...
void InferenceHelper::PreProcessByOpenCV(const InputTensorInfo& input_tensor_info, bool is_nchw,
                                         cv::Mat& img_blob) {
  /* Generate mat from original data */
  cv::Mat img_src = cv::Mat(
      cv::Size(input_tensor_info.image_info.width, input_tensor_info.image_info.height),
  ...
  if (input_tensor_info.image_info.width == input_tensor_info.image_info.crop_width &&
    ...
    img_src = img_src(cv::Rect(
        input_tensor_info.image_info.crop_x, input_tensor_info.image_info.crop_y,
        input_tensor_info.image_info.crop_width, input_tensor_info.image_info.crop_height));
  }
  ...
  if (input_tensor_info.image_info.crop_width == input_tensor_info.GetWidth() &&
    ...
    cv::resize(img_src, img_src,
               cv::Size(input_tensor_info.GetWidth(), input_tensor_info.GetHeight()));
  }
  ...
  if (input_tensor_info.image_info.channel == input_tensor_info.GetChannel()) {
    ...
    cv::cvtColor(img_src, img_src,
                 (input_tensor_info.image_info.is_bgr) ? cv::COLOR_BGR2GRAY : cv::COLOR_RGB2GRAY);
  ...
  if (input_tensor_info.tensor_type == TensorInfo::kTensorTypeFp32) {
    ...
    if (input_tensor_info.GetChannel() == 3) {
      ...
      cv::subtract(img_src, cv::Scalar(cv::Vec<float, 3>(input_tensor_info.normalize.mean)),
                   img_src);
      cv::multiply(img_src, cv::Scalar(cv::Vec<float, 3>(input_tensor_info.normalize.norm)),
                   img_src);
#else
      ...
      cv::subtract(img_src, cv::Scalar(cv::Vec<float, 1>(input_tensor_info.normalize.mean)),
                   img_src);
      cv::multiply(img_src, cv::Scalar(cv::Vec<float, 1>(input_tensor_info.normalize.norm)),
                   img_src);
#else
...
#endif
#if 1
  /* Convert to speeden up normalization:  ((src / 255) - mean) / norm = (src  - (mean * 255))  * (1
   * / (255 * norm)) */
  for (int32_t i = 0; i < 3; i++) {
...
void InferenceHelper::PreProcessImage(int32_t num_thread, const InputTensorInfo& input_tensor_info,
                                      float* dst) {
...
#else
        dst[i * img_channel + c] =
            (src[i * img_channel + c] / 255.0f - input_tensor_info.normalize.mean[c]) /
            input_tensor_info.normalize.norm[c];
#endif
      }
    }
...
template <typename T>
void InferenceHelper::PreProcessBlob(int32_t num_thread, const InputTensorInfo& input_tensor_info,
                                     T* dst) {
  const int32_t img_width = input_tensor_info.GetWidth();
  const int32_t img_height = input_tensor_info.GetHeight();
  const int32_t img_channel = input_tensor_info.GetChannel();
...
template void InferenceHelper::PreProcessBlob<float>(int32_t num_thread,
                                                     const InputTensorInfo& input_tensor_info,
                                                     float* dst);
template void InferenceHelper::PreProcessBlob<int32_t>(int32_t num_thread,
                                                       const InputTensorInfo& input_tensor_info,
                                                       int32_t* dst);
template void InferenceHelper::PreProcessBlob<int64_t>(int32_t num_thread,
                                                       const InputTensorInfo& input_tensor_info,
                                                       int64_t* dst);
template void InferenceHelper::PreProcessBlob<uint8_t>(int32_t num_thread,
                                                       const InputTensorInfo& input_tensor_info,
                                                       uint8_t* dst);
template void InferenceHelper::PreProcessBlob<int8_t>(int32_t num_thread,
                                                      const InputTensorInfo& input_tensor_info,
                                                      int8_t* dst);
