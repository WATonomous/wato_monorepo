...
class TensorInfo {
 ...
 public:
  ...
  std::vector<int32_t>
      tensor_dims;  // InputTensorInfo:   [In] The dimentions of tensor. (If empty at initialize,
                    // the size is updated from model info.) OutputTensorInfo: [Out] The dimentions
                    // of tensor is set from model information
...
class InputTensorInfo : public TensorInfo {
 public:
  enum {
    kDataTypeImage,
    kDataTypeBlobNhwc,  // data_ which already finished preprocess(color conversion, resize,
                        // normalize_, etc.)
    kDataTypeBlobNchw,
  };
...
class OutputTensorInfo : public TensorInfo {
 public:
  ...
  float* GetDataAsFloat() { /* Returned pointer should be with const, but returning pointer without
                               const is convenient to create cv::Mat */
...
class InferenceHelper {
 ...
 public:
  ...
  static void PreProcessByOpenCV(
      const InputTensorInfo& input_tensor_info, bool is_nchw,
 ...
 public:
  ...
  virtual int32_t SetCustomOps(
      const std::vector<std::pair<const char*, const void*>>& custom_ops) = 0;
