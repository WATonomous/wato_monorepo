...
/* ★ Modify the following (use the same parameter as the model. Also, ppm must be the same size but
 * not normalized.) */
...
int32_t InferenceHelperTensorRt::Initialize(
    const std::string& model_filename, std::vector<InputTensorInfo>& input_tensor_info_list,
...
int32_t InferenceHelperTensorRt::Process(std::vector<OutputTensorInfo>& output_tensor_info_list) {
  ...
  for (int i = 0; i < (int)buffer_list_cpu_.size(); i++) {
    if (engine_->bindingIsInput(i)) {
      cudaMemcpyAsync(buffer_list_gpu_[i], buffer_list_cpu_[i].first, buffer_list_cpu_[i].second,
  ...
  for (int i = 0; i < (int)buffer_list_cpu_.size(); i++) {
    if (!engine_->bindingIsInput(i)) {
      cudaMemcpyAsync(buffer_list_cpu_[i].first, buffer_list_gpu_[i], buffer_list_cpu_[i].second,
  ...
  (void)output_tensor_info_list;  // no need to set output data, because the ptr to output data is
                                  // already set at initialize
