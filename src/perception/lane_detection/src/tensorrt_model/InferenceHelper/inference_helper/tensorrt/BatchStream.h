...
class MNISTBatchStream : public IBatchStream {
 public:
  MNISTBatchStream(int batchSize, int maxBatches, const std::string& dataFile,
                   const std::string& labelsFile, const std::vector<std::string>& directories)
  ...
  float* getLabels() override { return mLabels.data() + (mBatchCount * mBatchSize); }
 ...
 private:
  void readDataFile(const std::string& dataFilePath) {
    ...
    ASSERT(magicNumber == 2051 &&
           "Magic Number does not match the expected value for an MNIST image set");
  ...
  void readLabelsFile(const std::string& labelsFilePath) {
    ...
    ASSERT(magicNumber == 2049 &&
           "Magic Number does not match the expected value for an MNIST labels file");
...
class BatchStream : public IBatchStream {
 public:
  BatchStream(int batchSize, int maxBatches, std::string prefix, std::string suffix,
  ...
  BatchStream(int batchSize, int maxBatches, nvinfer1::Dims dims, std::string listFile,
              std::vector<std::string> directories)
      : mBatchSize(batchSize),
        mMaxBatches(maxBatches),
        mDims(dims),
  ...
  bool next() override {
    ...
    for (int csize = 1, batchPos = 0; batchPos < mBatchSize;
      ...
      // copy the smaller of: elements left to fulfill the request, or elements left in the file
      // buffer.
  ...
  bool update() {
    if (mListFile.empty()) {
      std::string inputFileName =
          locateFile(mPrefix + std::to_string(mFileCount++) + mSuffix, mDataDir);
