...
#define CHECK_RETURN_W_MSG(status, val, errMsg)                                           \
  do {                                                                                    \
    if (!(status)) {                                                                      \
      sample::gLogError << errMsg << " Error in " << __FILE__ << ", function " << FN_NAME \
...
std::vector<std::string> classify(const std::vector<std::string>& refVector,
                                  const std::vector<T>& output, const size_t topK) {
...
inline void setDummyInt8DynamicRanges(const IBuilderConfig* c, INetworkDefinition* n) {
  ...
  if (c->getFlag(BuilderFlag::kINT8)) {
    sample::gLogWarning << "Int8 calibrator not provided. Generating dummy per-tensor dynamic "
...
#else
    sample::gLogError << "Could not load plugin library: " << path << ", due to: " << dlerror()
                      << std::endl;
#endif
  }
}
