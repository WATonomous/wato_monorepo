...
vision_msgs::msg::Detection3DArray arraysToDetection3DArray(
  ...
  const std::vector<std::array<float, 8>> & det_info, const std::vector<int> & class_ids)
{
  ...
  for (size_t i = 0; i < det_info.size(); ++i) {
    ...
    strks.push_back(std::make_shared<byte_track::STrack>(rect, d[7], class_ids[i]));
  }
  ...
  SECTION("Required data matches")
  {
    for (size_t i = 0; i < objs.size(); ++i) {
      ...
      for (size_t j = 0; j < 7; ++j) {
        if (j == 3) {
          REQUIRE_THAT(principleAngle(obj.rect.xyzolwh[j]), Catch::Matchers::WithinRel(principleAngle(exp[j])));
        } else {
...
TEST_CASE("STrack to Detection3DArray conversion before publishing", "[conv_2]")
{
  ...
  SECTION("Required data matches")
  {
    for (size_t i = 0; i < trks.detections.size(); ++i) {
      ...
      REQUIRE_THAT(
        principleAngle(tf2::getYaw(trk_pos.orientation)), Catch::Matchers::WithinRel(principleAngle(exp[3])));
