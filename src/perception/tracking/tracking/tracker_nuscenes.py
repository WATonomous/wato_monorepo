...
class trackerNode(Node):
    def __init__(self, config_path, publish_frequency):
        ...
        self.detection_subscriber = self.create_subscription(MarkerArray, '/markers/annotations', self.detection_callback, 10)
    ...
    def detection_callback(self, msg):
        ...
        informations = []		# [class label, confidence score] empty list to store the classifcation information for each detected obstacels 

        for marker in msg.markers: #Iterates through each detection
            detections.append(ros_utils.obstacle_to_bbox(marker)) # Converts the bonding box information from the Detection3D message into the required format mentioned above for detections
            informations.append(["car", 1.0]) #Extracts the calss lable of the first hypothesis from the detection results then extracts the confidence score and appends a list containg the class lale and confidece score to infromations list
