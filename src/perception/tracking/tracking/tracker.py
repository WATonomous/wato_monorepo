...
class trackerNode(Node):
    ...
    def publish_tracks(self, dt):
        ...
        #         f"Published tracked obstacles: ||| {len(tracked_obstacle_list.tracked_obstacles)}; {tracked_obstacle_list.tracked_obstacles[0].obstacle.twist.twist.linear.x}, {tracked_obstacle_list.tracked_obstacles[0].obstacle.twist.twist.linear.y}; {tracked_obstacle_list.tracked_obstacles[0].obstacle.pose.pose.position.x}, {tracked_obstacle_list.tracked_obstacles[0].obstacle.pose.pose.position.y}"  # noqa: E501
        #     )
    ...
    def _create_tracked_obstacle_message(self, kf_track, tracking_name, track_score, dt):
        ...
        for timestamp, history in reversed(kf_track.history):  # Iterates through the list of timestamps in reverse order  # noqa: E501
            ...
            if dt >= self.velocity_prediction_horizon:  # Ensures states separate by significant time interval are considered for estimation  # noqa: E501
                # Ignore z velocity.
                # Performs calculation to estimate new velocity for the current track
        ...
        # Create observation history messages. Creates another history list
        # of trackers so that it can be used by ROS
...
def main(args=None):
    ...
    try:
        with open(
            os.path.join(
                get_package_share_directory("tracking"), "config", "mahalanobis.yaml"
            ),
            "r",
        ):
            pass
        ...
        )  # Used to change type of matching algorithm to be used
    ...
    try:
        while rclpy.ok():
            ...
            #     tracker_node.get_logger().error(
            #         f"Error: Exception caught while processing a frame: {e}"
            #     )
            try:
                # Node has 1.25 seconds to process and publish messages
                rclpy.spin_once(tracker_node, timeout_sec=1.0 / 0.8)
