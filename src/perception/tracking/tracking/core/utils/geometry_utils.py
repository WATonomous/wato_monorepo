...
def project_bbox_2d(bbox, to_camera_transform, intrinsic_matrix):
    ...
    points_cam_norm = points_cam[:,:2] / points_cam[:,2].reshape(-1, 1)                     # Normalize by z coordinate

    min_xy = np.min(points_cam_norm, axis=0)
    max_xy = np.max(points_cam_norm, axis=0)
    dimensions = max_xy - min_xy
    if (np.any(dimensions == 0) or np.any(points_cam[:,2] > 100)): # Clip boxes outside the camera image
        raise ClippingError()
...
def transform_points(points, transform):
    """
    Transforms a series of points based on a transformationStamped
    ...
    translation = tr.translation_matrix([transform.translation.x, transform.translation.y, transform.translation.z])
