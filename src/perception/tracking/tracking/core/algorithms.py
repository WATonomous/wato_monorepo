...
def mahalanobis_metric(detections, trackers, **kwargs):
    """
    Create matrix of mahalanobis distances between detections and tracks.
...
def iou_metric(detections, trackers, **kwargs):
    """
    Create matrix of negative IOU score between detections and tracks.
    ...
    for d, det in enumerate(detections):
        for t, trk in enumerate(trackers):
            det_8corner = convert_3dbox_to_8corner(det)
            trk_8corner = convert_3dbox_to_8corner(trk)
            score_matrix[d, t] = -iou3d(det_8corner, trk_8corner)[0]

    return score_matrix


def euclidian_distance(detections, trackers, **kwargs):
    """
    Create matrix of euclidian distance score between detections and tracks (in meters).
    ...
    for d, det in enumerate(detections):
        for t, trk in enumerate(trackers):
            score_matrix[d, t] = (
                (det[0] - trk[0]) ** 2 + (det[1] - trk[1]) ** 2 + (det[3] - trk[3]) ** 2
            ) ** 0.5

    return score_matrix


def mahalanobis_euclidian_metric(detections, trackers, **kwargs):
    return euclidian_distance(detections, trackers, **kwargs) + \
        mahalanobis_metric(detections, trackers, **kwargs)
