...
def euclidian_distance(detections, trackers, **kwargs):
  """
    Creates matrix of euclidian distance score between detections and tracks (in meters)
  ...
  for d,det in enumerate(detections):
    for t,trk in enumerate(trackers):
      score_matrix[d,t] = ((det[0] - trk[0])**2 + (det[1] - trk[1])**2 + (det[3] - trk[3])**2)**0.5

  return score_matrix

def mahalanobis_euclidian_metric(detections, trackers, **kwargs):
  return euclidian_distance(detections, trackers, **kwargs) + mahalanobis_metric(detections, trackers, **kwargs)
