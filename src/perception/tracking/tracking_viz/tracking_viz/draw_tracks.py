    ...
    def try_draw(self):
        ...
        for det_msg in det_3d_msg.detections:
            ...
            for v in vert:
                ...
                v_trans = np.array(
                    [v_trans.position.x, v_trans.position.y, v_trans.position.z, 1]
                )
            ...
            for i in range(4):
                image = cv2.line(
                    image, verts_2d[i], verts_2d[(i + 1) % 4], color, 10
                )  # face 1
                image = cv2.line(
                    image, verts_2d[i + 4], verts_2d[(i + 1) % 4 + 4], color, 10
                )  # face 2
                image = cv2.line(
                    image, verts_2d[i], verts_2d[i + 4], color, 10
                )  # connect faces
