...
TrackingNode::TrackingNode()
    : Node("dets_2d_3d", rclcpp::NodeOptions()
...
void TrackingNode::receiveDetections(const vision_msgs::msg::Detection2DArray::SharedPtr msg) {
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr lidarCloudNoFloor(new pcl::PointCloud<pcl::PointXYZ>());
  {
    ...
    RCLCPP_INFO(this->get_logger(), "receive detection %ld vs %ld", lidarCloud_->size(),
                lidarCloudNoFloor->size());
  }
...
int TrackingNode::highestIOUScoredBBox(const std::vector<vision_msgs::msg::BoundingBox3D> bboxes,
  ...
  for (size_t i = 0; i < bboxes.size(); ++i) {
    ...
    std::optional<geometry_msgs::msg::Point> top_left2d =
        ProjectionUtils::projectLidarToCamera(transform_, camInfo_->p, top_left);
    std::optional<geometry_msgs::msg::Point> bottom_right2d =
        ProjectionUtils::projectLidarToCamera(transform_, camInfo_->p, bottom_right);
...
double TrackingNode::overlapBoundingBox(const vision_msgs::msg::BoundingBox2D &boxA,
  ...
  double overlapWidth =
      std::min(boxA.center.position.x + boxA.size_x / 2, boxB.center.position.x + boxB.size_x / 2) -
...
double TrackingNode::iouScore(const vision_msgs::msg::BoundingBox2D &bboxA,
  ...
  return (overlap / (bboxA.size_x * bboxA.size_y + bboxB.size_x * bboxB.size_y - overlap));
}
