...
HAL_StatusTypeDef HAL_DMA_Init(DMA_HandleTypeDef * hdma)
{
  ...
  if (IS_DMA_STREAM_INSTANCE(hdma->Instance) != 0U) /* DMA1 or DMA2 instance */ {
    ...
    registerValue &= ((uint32_t)~(
      DMA_SxCR_MBURST | DMA_SxCR_PBURST | DMA_SxCR_PL | DMA_SxCR_MSIZE | DMA_SxCR_PSIZE | DMA_SxCR_MINC |
    ...
    registerValue &= ((uint32_t)~(
      BDMA_CCR_PL | BDMA_CCR_MSIZE | BDMA_CCR_PSIZE | BDMA_CCR_MINC | BDMA_CCR_PINC | BDMA_CCR_CIRC | BDMA_CCR_DIR |
      BDMA_CCR_MEM2MEM | BDMA_CCR_DBM | BDMA_CCR_CT));

    /* Prepare the DMA Channel configuration */
    registerValue |= DMA_TO_BDMA_DIRECTION(hdma->Init.Direction) | DMA_TO_BDMA_PERIPHERAL_INC(hdma->Init.PeriphInc) |
...
void HAL_DMA_IRQHandler(DMA_HandleTypeDef * hdma)
{
  ...
  if (IS_DMA_STREAM_INSTANCE(hdma->Instance) != 0U) /* DMA1 or DMA2 instance */ {
    ...
    /* Transfer Complete Interrupt management ***********************************/
      ...
      ((tmpisr_bdma & (BDMA_FLAG_TC0 << (hdma->StreamIndex & 0x1FU))) != 0U) && ((ccr_reg & BDMA_CCR_TCIE) != 0U))
    {
      /* Clear the transfer complete flag */
    ...
    /* Transfer Error Interrupt management **************************************/
      ...
      ((tmpisr_bdma & (BDMA_FLAG_TE0 << (hdma->StreamIndex & 0x1FU))) != 0U) && ((ccr_reg & BDMA_CCR_TEIE) != 0U))
    {
      /* When a DMA transfer error occurs */
      /* A hardware clear of its EN bits is performed */
      /* Disable ALL DMA IT */
  ...
  if ((request >= DMA_REQUEST_GENERATOR0) && (request <= DMA_REQUEST_GENERATOR7)) {
    if (IS_BDMA_CHANNEL_DMAMUX_INSTANCE(hdma->Instance) != 0U) {
      ...
      hdma->DMAmuxRequestGen =
        (DMAMUX_RequestGen_TypeDef *)((uint32_t)(((uint32_t)DMAMUX2_RequestGenerator0) + ((request - 1U) * 4U)));
