...
HAL_StatusTypeDef HAL_MDMA_LinkedList_CreateNode(MDMA_LinkNodeTypeDef * pNode, MDMA_LinkNodeConfTypeDef * pNodeConfig)
{
  ...
  pNode->CTCR = pNodeConfig->Init.SourceInc | pNodeConfig->Init.DestinationInc | pNodeConfig->Init.SourceDataSize |
                pNodeConfig->Init.DestDataSize | pNodeConfig->Init.DataAlignment | pNodeConfig->Init.SourceBurst |
                pNodeConfig->Init.DestBurst | ((pNodeConfig->Init.BufferTransferLength - 1U) << MDMA_CTCR_TLEN_Pos) |
...
HAL_StatusTypeDef HAL_MDMA_LinkedList_RemoveNode(MDMA_HandleTypeDef * hmdma, MDMA_LinkNodeTypeDef * pNode)
{
  ...
  if (HAL_MDMA_STATE_READY == hmdma->State) {
    ...
    if (
      ((uint32_t)hmdma->FirstLinkedListNodeAddress == 0U) || ((uint32_t)hmdma->LastLinkedListNodeAddress == 0U) ||
...
static void MDMA_Init(MDMA_HandleTypeDef * hmdma)
{
  ...
  /* Write new CTCR Register value */
  hmdma->Instance->CTCR = hmdma->Init.SourceInc | hmdma->Init.DestinationInc | hmdma->Init.SourceDataSize |
                          hmdma->Init.DestDataSize | hmdma->Init.DataAlignment | hmdma->Init.SourceBurst |
                          hmdma->Init.DestBurst | ((hmdma->Init.BufferTransferLength - 1U) << MDMA_CTCR_TLEN_Pos) |
