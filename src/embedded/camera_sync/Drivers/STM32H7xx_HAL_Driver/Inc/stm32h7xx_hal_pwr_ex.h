...
#define PWR_WAKEUP_FLAG6 PWR_WKUPFR_WKUPF6 /*!< Wakeup flag on PC1  */
  ...
  #define PWR_WAKEUP_FLAG_ALL                                                                            \
    (PWR_WKUPFR_WKUPF1 | PWR_WKUPFR_WKUPF2 | PWR_WKUPFR_WKUPF3 | PWR_WKUPFR_WKUPF4 | PWR_WKUPFR_WKUPF5 | \
     PWR_WKUPFR_WKUPF6)
#else
  #define PWR_WAKEUP_FLAG_ALL (PWR_WKUPFR_WKUPF1 | PWR_WKUPFR_WKUPF2 | PWR_WKUPFR_WKUPF4 | PWR_WKUPFR_WKUPF6)
#endif /* defined (PWR_WKUPFR_WKUPF3) */
  /**
  * @}
  */
...
#define PWR_EXTERNAL_SOURCE_SUPPLY \
  PWR_CR3_BYPASS /*!< The SMPS disabled and the LDO Bypass. The Core domains are supplied from an external source                 */
...
#define PWR_EXTI_LINE_AVD \
  ...
  */
  ...
  #define PWR_USB_FDCAN_MEMORY_BLOCK \
    PWR_CR1_HSITFSO /*!< High-speed interfaces USB and FDCAN memories shut-off in DStop/DStop2 mode */
...
/* Check PWR regulator configuration parameter */
  ...
  #define IS_PWR_SUPPLY(PWR_SOURCE)                                                                                \
    (((PWR_SOURCE) == PWR_LDO_SUPPLY) || ((PWR_SOURCE) == PWR_DIRECT_SMPS_SUPPLY) ||                               \
     ((PWR_SOURCE) == PWR_SMPS_1V8_SUPPLIES_LDO) || ((PWR_SOURCE) == PWR_SMPS_2V5_SUPPLIES_LDO) ||                 \
     ((PWR_SOURCE) == PWR_SMPS_1V8_SUPPLIES_EXT_AND_LDO) || ((PWR_SOURCE) == PWR_SMPS_2V5_SUPPLIES_EXT_AND_LDO) || \
     ((PWR_SOURCE) == PWR_SMPS_1V8_SUPPLIES_EXT) || ((PWR_SOURCE) == PWR_SMPS_2V5_SUPPLIES_EXT) ||                 \
     ((PWR_SOURCE) == PWR_EXTERNAL_SOURCE_SUPPLY))

#else
  #define IS_PWR_SUPPLY(PWR_SOURCE) (((PWR_SOURCE) == PWR_LDO_SUPPLY) || ((PWR_SOURCE) == PWR_EXTERNAL_SOURCE_SUPPLY))
#endif /* defined (SMPS) */
...
/* Check wake up pin parameter */
  ...
  #define IS_PWR_WAKEUP_PIN(PIN)                                                                              \
    (((PIN) == PWR_WAKEUP_PIN1) || ((PIN) == PWR_WAKEUP_PIN2) || ((PIN) == PWR_WAKEUP_PIN3) ||                \
     ...
     ((PIN) == PWR_WAKEUP_PIN1_HIGH) || ((PIN) == PWR_WAKEUP_PIN2_HIGH) || ((PIN) == PWR_WAKEUP_PIN3_HIGH) || \
     ((PIN) == PWR_WAKEUP_PIN4_HIGH) || ((PIN) == PWR_WAKEUP_PIN5_HIGH) || ((PIN) == PWR_WAKEUP_PIN6_HIGH) || \
     ((PIN) == PWR_WAKEUP_PIN1_LOW) || ((PIN) == PWR_WAKEUP_PIN2_LOW) || ((PIN) == PWR_WAKEUP_PIN3_LOW) ||    \
     ((PIN) == PWR_WAKEUP_PIN4_LOW) || ((PIN) == PWR_WAKEUP_PIN5_LOW) || ((PIN) == PWR_WAKEUP_PIN6_LOW))
#else
  #define IS_PWR_WAKEUP_PIN(PIN)                                                                             \
    (((PIN) == PWR_WAKEUP_PIN1) || ((PIN) == PWR_WAKEUP_PIN2) || ((PIN) == PWR_WAKEUP_PIN4) ||               \
     ((PIN) == PWR_WAKEUP_PIN6) || ((PIN) == PWR_WAKEUP_PIN1_HIGH) || ((PIN) == PWR_WAKEUP_PIN2_HIGH) ||     \
     ((PIN) == PWR_WAKEUP_PIN4_HIGH) || ((PIN) == PWR_WAKEUP_PIN6_HIGH) || ((PIN) == PWR_WAKEUP_PIN1_LOW) || \
     ((PIN) == PWR_WAKEUP_PIN2_LOW) || ((PIN) == PWR_WAKEUP_PIN4_LOW) || ((PIN) == PWR_WAKEUP_PIN6_LOW))
#endif /* defined (PWR_WKUPEPR_WKUPEN3) */
...
#else
  #define IS_PWR_WAKEUP_FLAG(FLAG)                                                                   \
    (((FLAG) == PWR_WAKEUP_FLAG1) || ((FLAG) == PWR_WAKEUP_FLAG2) || ((FLAG) == PWR_WAKEUP_FLAG4) || \
...
#define IS_PWR_D1_CPU(CPU) ((CPU) == CM7_CPUID)
  ...
  #define IS_PWR_DOMAIN_FLAG(FLAG) \
    (((FLAG) == PWR_D1_DOMAIN_FLAGS) || ((FLAG) == PWR_D2_DOMAIN_FLAGS) || ((FLAG) == PWR_ALL_DOMAIN_FLAGS))
#endif /* defined (DUAL_CORE) */
