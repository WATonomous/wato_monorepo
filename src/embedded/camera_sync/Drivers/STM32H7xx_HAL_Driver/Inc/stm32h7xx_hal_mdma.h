...
#include "stm32h7xx_hal_def.h"
  ...
  */
  typedef struct
  {
    ...
    uint32_t
      PostRequestMaskAddress; /*!< specifies the address to be updated (written) with PostRequestMaskData after a request is served.
    ...
    uint32_t
      PostRequestMaskData; /*!< specifies the value to be written to PostRequestMaskAddress after a request is served.
                                         PostRequestMaskAddress and PostRequestMaskData could be used to automatically clear a peripheral flag when the request is served  */
...
#define MDMA_REPEAT_BLOCK_TRANSFER \
  ((uint32_t)                      \
     MDMA_CTCR_TRGM_1) /*!< Each MDMA request (SW or HW) triggers a repeated block transfer                        */
#define MDMA_FULL_TRANSFER \
  ((uint32_t)              \
     MDMA_CTCR_TRGM) /*!< Each MDMA request (SW or HW) triggers a Full transfer or a linked list transfer if any */
...
#define MDMA_LITTLE_BYTE_ENDIANNESS_EXCHANGE \
  ((uint32_t)MDMA_CCR_BEX) /*!< BYTEs endianness exchange when destination data size is > Byte           */
#define MDMA_LITTLE_HALFWORD_ENDIANNESS_EXCHANGE \
  ((uint32_t)MDMA_CCR_HEX) /*!< HALF WORDs endianness exchange when destination data size is > HALF WORD */
#define MDMA_LITTLE_WORD_ENDIANNESS_EXCHANGE \
  ((uint32_t)MDMA_CCR_WEX) /*!< WORDs endianness exchange  when destination data size is > DOUBLE WORD   */
...
#define MDMA_SRC_INC_HALFWORD   \
  ((uint32_t)MDMA_CTCR_SINC_1 | \
   (uint32_t)MDMA_CTCR_SINCOS_0) /*!< Source address pointer is incremented by a half Word (16 bits)    */
#define MDMA_SRC_INC_WORD       \
  ((uint32_t)MDMA_CTCR_SINC_1 | \
   (uint32_t)MDMA_CTCR_SINCOS_1) /*!< Source address pointer is incremented by a Word (32 bits)         */
#define MDMA_SRC_INC_DOUBLEWORD \
  ((uint32_t)MDMA_CTCR_SINC_1 | \
   (uint32_t)MDMA_CTCR_SINCOS) /*!< Source address pointer is incremented by a double Word (64 bits)) */
...
#define MDMA_SRC_DEC_HALFWORD \
  ((uint32_t)MDMA_CTCR_SINC | \
   (uint32_t)MDMA_CTCR_SINCOS_0) /*!< Source address pointer is decremented by a half Word (16 bits)    */
#define MDMA_SRC_DEC_WORD     \
  ((uint32_t)MDMA_CTCR_SINC | \
   (uint32_t)MDMA_CTCR_SINCOS_1) /*!< Source address pointer is decremented by a Word (32 bits)         */
#define MDMA_SRC_DEC_DOUBLEWORD \
  ((uint32_t)MDMA_CTCR_SINC |   \
   (uint32_t)MDMA_CTCR_SINCOS) /*!< Source address pointer is decremented by a double Word (64 bits)) */
...
#define MDMA_DEST_INC_HALFWORD  \
  ((uint32_t)MDMA_CTCR_DINC_1 | \
   (uint32_t)MDMA_CTCR_DINCOS_0) /*!< Source address pointer is incremented by a half Word (16 bits)    */
#define MDMA_DEST_INC_WORD      \
  ((uint32_t)MDMA_CTCR_DINC_1 | \
   (uint32_t)MDMA_CTCR_DINCOS_1) /*!< Source address pointer is incremented by a Word (32 bits)         */
#define MDMA_DEST_INC_DOUBLEWORD \
  ((uint32_t)MDMA_CTCR_DINC_1 |  \
   (uint32_t)MDMA_CTCR_DINCOS) /*!< Source address pointer is incremented by a double Word (64 bits)) */
...
#define MDMA_DEST_DEC_HALFWORD \
  ((uint32_t)MDMA_CTCR_DINC |  \
   (uint32_t)MDMA_CTCR_DINCOS_0) /*!< Source address pointer is decremented by a half Word (16 bits)    */
#define MDMA_DEST_DEC_WORD    \
  ((uint32_t)MDMA_CTCR_DINC | \
   (uint32_t)MDMA_CTCR_DINCOS_1) /*!< Source address pointer is decremented by a Word (32 bits)         */
#define MDMA_DEST_DEC_DOUBLEWORD \
  ((uint32_t)MDMA_CTCR_DINC |    \
   (uint32_t)MDMA_CTCR_DINCOS) /*!< Source address pointer is decremented by a double Word (64 bits)) */
...
#define MDMA_DATAALIGN_PACKENABLE \
  ((uint32_t)MDMA_CTCR_PKE) /*!< The source data is packed/un-packed into the destination data size
...
#define __HAL_MDMA_GET_COUNTER(__HANDLE__) ((__HANDLE__)->Instance->CBNDTR & MDMA_CBNDTR_BNDT)
  ...
  */
  ...
  HAL_StatusTypeDef HAL_MDMA_RegisterCallback(
    MDMA_HandleTypeDef * hmdma, HAL_MDMA_CallbackIDTypeDef CallbackID, void (*pCallback)(MDMA_HandleTypeDef * _hmdma));
  ...
  */
  ...
  HAL_StatusTypeDef HAL_MDMA_LinkedList_AddNode(
    MDMA_HandleTypeDef * hmdma, MDMA_LinkNodeTypeDef * pNewNode, const MDMA_LinkNodeTypeDef * pPrevNode);
...
#define IS_MDMA_LEVEL_COMPLETE(__LEVEL__)                                                  \
  (((__LEVEL__) == HAL_MDMA_FULL_TRANSFER) || ((__LEVEL__) == HAL_MDMA_BUFFER_TRANSFER) || \
   ((__LEVEL__) == HAL_MDMA_BLOCK_TRANSFER) || ((__LEVEL__) == HAL_MDMA_REPEAT_BLOCK_TRANSFER))
...
#define IS_MDMA_SOURCE_INC(__INC__)                                                                                   \
  (((__INC__) == MDMA_SRC_INC_DISABLE) || ((__INC__) == MDMA_SRC_INC_BYTE) || ((__INC__) == MDMA_SRC_INC_HALFWORD) || \
   ((__INC__) == MDMA_SRC_INC_WORD) || ((__INC__) == MDMA_SRC_INC_DOUBLEWORD) || ((__INC__) == MDMA_SRC_DEC_BYTE) ||  \
...
#define IS_MDMA_DATA_ALIGNMENT(__ALIGNMENT__)                                                     \
  (((__ALIGNMENT__) == MDMA_DATAALIGN_PACKENABLE) || ((__ALIGNMENT__) == MDMA_DATAALIGN_RIGHT) || \
   ((__ALIGNMENT__) == MDMA_DATAALIGN_RIGHT_SIGNED) || ((__ALIGNMENT__) == MDMA_DATAALIGN_LEFT))
...
#define IS_MDMA_SOURCE_BURST(__BURST__)                                                        \
  (((__BURST__) == MDMA_SOURCE_BURST_SINGLE) || ((__BURST__) == MDMA_SOURCE_BURST_2BEATS) ||   \
   ...
   ((__BURST__) == MDMA_SOURCE_BURST_16BEATS) || ((__BURST__) == MDMA_SOURCE_BURST_32BEATS) || \
   ((__BURST__) == MDMA_SOURCE_BURST_64BEATS) || ((__BURST__) == MDMA_SOURCE_BURST_128BEATS))
