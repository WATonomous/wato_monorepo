...
static int init_can()
{
  ...
  if (ret == NOERR) {
    ret = check("canSetBusParams", canSetBusParams(can_handle, BAUD_500K, 0, 0, 0, 0, 0));
  }

  if (ret == NOERR) {
    ret = check("canSetBusOutputControl", canSetBusOutputControl(can_handle, canDRIVER_NORMAL));
  }
...
static int read_from_buffer(canHandle handle)
{
  ...
  retval = canRead(handle, &can_id, buffer, &msg_dlc, &msg_flag, &tstamp);
