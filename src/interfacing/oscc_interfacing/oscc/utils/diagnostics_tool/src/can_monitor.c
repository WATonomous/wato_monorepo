  ...
  const unsigned char * const buffer)
{
  ...
  return update_can_array_contents(msg_dlc, msg_flag, buffer, &can_msg_array[msg_array_index].frame_contents);
}
...
static int get_can_msg_array_index(const long can_id)
{
  ...
  for (i = 0; i < CAN_MSG_ARRAY_SIZE; i++) {
    ...
    if (can_msg_array[i].can_id == 0 || can_msg_array[i].can_id == can_id) {
      msg_array_index = i;
...
//
static int calc_msg_freq(const int deltaT_message_arrival, const int array_index)
{
...
//
static int calc_msg_timestamp_freq(const int deltaT_message_arrival, const int array_index)
{
  const float factor = 0.4;
...
//
static void update_can_array_timestamp(const int array_index, const unsigned long tstamp)
{
  int deltaT_arrival = get_timestamp() - can_msg_array[array_index].last_arrival_timestamp;

  can_msg_array[array_index].last_arrival_timestamp = get_timestamp();

  int msg_timestamp_deltaT_arrival = tstamp - can_msg_array[array_index].frame_contents.tstamp;

  can_msg_array[array_index].frame_contents.tstamp = tstamp;

  can_msg_array[array_index].last_msg_deltaT = deltaT_arrival;

  can_msg_array[array_index].last_msg_timestamp_deltaT = msg_timestamp_deltaT_arrival;

  can_msg_array[array_index].msg_frequency = calc_msg_freq(deltaT_arrival, array_index);
  ...
  const unsigned char * const buffer)
{
  ...
  if (ret == NOERR) {
    int new_msg_contents = update_can_array_msg(can_msg_array_index, can_id, msg_dlc, msg_flag, buffer);

    update_can_array_timestamp(can_msg_array_index, tstamp);
  }
