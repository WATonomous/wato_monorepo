...
static int update_steering_state()
{
  ...
  const can_frame_s * const steering_command_frame = get_can_msg_array_index_reference(OSCC_STEERING_COMMAND_CAN_ID);

  const can_frame_s * const steering_report_frame = get_can_msg_array_index_reference(OSCC_STEERING_REPORT_CAN_ID);

  if (steering_command_frame != NULL || steering_report_frame != NULL) {
    ret = analyze_steering_state(&system_state.steering_module_state, steering_command_frame, steering_report_frame);
  }
...
static int update_throttle_state()
{
  ...
  const can_frame_s * const throttle_command_frame = get_can_msg_array_index_reference(OSCC_THROTTLE_COMMAND_CAN_ID);

  const can_frame_s * const throttle_report_frame = get_can_msg_array_index_reference(OSCC_THROTTLE_REPORT_CAN_ID);

  if (throttle_command_frame != NULL || throttle_report_frame != NULL) {
    ret = analyze_throttle_state(&system_state.throttle_module_state, throttle_command_frame, throttle_report_frame);
  }
...
static int update_brake_state()
{
  ...
  const can_frame_s * const brake_command_frame = get_can_msg_array_index_reference(OSCC_BRAKE_COMMAND_CAN_ID);

  const can_frame_s * const brake_report_frame = get_can_msg_array_index_reference(OSCC_BRAKE_REPORT_CAN_ID);

  if (brake_command_frame != NULL || brake_report_frame != NULL) {
    ret = analyze_brake_state(&system_state.brake_module_state, brake_command_frame, brake_report_frame);
  }
