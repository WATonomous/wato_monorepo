...
#define ENSURE_VA_NUM_ARGS(...) \
  ENSURE_VA_NUM_ARGS_IMPL_(     \
    (__VA_ARGS__, _CALLED_WITH_TOO_MANY_ARGUMENTS, WithContextAndSpecificationName, WithSpecificationName))
...
#define EnsureWithContextAndSpecificationName(skip, contextName, specName, ...)                                 \
  ...
  CgreenTest spec_name(contextName, specName) = {                                                               \
    skip, &contextFor##contextName, STRINGIFY_TOKEN(specName), &contextName##__##specName, __FILE__, __LINE__}; \
  static void contextName##__##specName(void)
...
#define EnsureWithSpecificationName(skip, specName, ...)                              \
  ...
  CgreenTest spec_name(default, specName) = {                                         \
    skip, &defaultContext, STRINGIFY_TOKEN(specName), &specName, __FILE__, __LINE__}; \
  static void specName(void)
