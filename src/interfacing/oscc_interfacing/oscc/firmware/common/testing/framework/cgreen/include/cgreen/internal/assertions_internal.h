...
#define ASSERT_THAT_VA_NUM_ARGS(...) \
  ASSERT_THAT_VA_NUM_ARGS_IMPL_((__VA_ARGS__, _CALLED_WITH_TOO_MANY_ARGS, _constraint, _expression))
...
#define ASSERT_THAT_macro_dispatcher(func, ...) \
  ASSERT_THAT_macro_dispatcher_(func, ASSERT_THAT_VA_NUM_ARGS(__VA_ARGS__))
...
#define assert_that_expression(expression) \
  ...
  void assert_string_equal_(
    const char * file, int line, const char * expression, const char * tried, const char * expected);
  void assert_string_not_equal_(
    const char * file, int line, const char * expression, const char * tried, const char * expected);
  void assert_that_double_(
    const char * file, int line, const char * actual_string, double actual, Constraint * constraint);
