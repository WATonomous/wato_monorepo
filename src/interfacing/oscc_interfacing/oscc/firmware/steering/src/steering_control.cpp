...
static void read_torque_sensor(steering_torque_s * value);

static float exponential_moving_average(const float alpha, const float input, const float average);
...
void check_for_faults(void)
{
  ...
  if ((g_steering_control_state.enabled == true) || (g_steering_control_state.dtcs > 0)) {
    ...
    filtered_diff = exponential_moving_average(filter_alpha, unfiltered_diff, filtered_diff);
#endif

    bool inputs_grounded = check_voltage_grounded(torque.high, torque.low, FAULT_HYSTERESIS, &grounded_fault_state);
    ...
    else if (abs(filtered_diff) > TORQUE_DIFFERENCE_OVERRIDE_THRESHOLD)
    {
      ...
      DTC_SET(g_steering_control_state.dtcs, OSCC_STEERING_DTC_OPERATOR_OVERRIDE);
...
void update_steering(uint16_t spoof_command_high, uint16_t spoof_command_low)
{
  if (g_steering_control_state.enabled == true) {
    uint16_t spoof_high =
      constrain(spoof_command_high, STEERING_SPOOF_HIGH_SIGNAL_RANGE_MIN, STEERING_SPOOF_HIGH_SIGNAL_RANGE_MAX);

    uint16_t spoof_low =
      constrain(spoof_command_low, STEERING_SPOOF_LOW_SIGNAL_RANGE_MIN, STEERING_SPOOF_LOW_SIGNAL_RANGE_MAX);
...
{
  if (g_steering_control_state.enabled == false && g_steering_control_state.operator_override == false) {
    const uint16_t num_samples = 20;
    prevent_signal_discontinuity(g_dac, num_samples, PIN_TORQUE_SENSOR_HIGH, PIN_TORQUE_SENSOR_LOW);
