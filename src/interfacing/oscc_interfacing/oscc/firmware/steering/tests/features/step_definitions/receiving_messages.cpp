...
WHEN("^an enable steering command is received$")
{
  ...
  oscc_steering_enable_s * steering_enable = (oscc_steering_enable_s *)g_mock_mcp_can_read_msg_buf_buf;
...
WHEN("^a disable steering command is received$")
{
  ...
  oscc_steering_disable_s * steering_disable = (oscc_steering_disable_s *)g_mock_mcp_can_read_msg_buf_buf;
...
WHEN("^a command is received with request value (.*)$")
{
  ...
  oscc_steering_command_s * steering_command = (oscc_steering_command_s *)g_mock_mcp_can_read_msg_buf_buf;
