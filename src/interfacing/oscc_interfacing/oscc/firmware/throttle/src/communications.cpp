...
static void process_throttle_command(const uint8_t * const data);
...
void publish_throttle_report(void)
{
  ...
  g_control_can.sendMsgBuf(
    OSCC_THROTTLE_REPORT_CAN_ID, CAN_STANDARD, OSCC_THROTTLE_REPORT_CAN_DLC, (uint8_t *)&throttle_report);
  sei();
}
...
{
  if (data != NULL) {
    const oscc_throttle_command_s * const throttle_command = (oscc_throttle_command_s *)data;

    const float clamped_position =
      CONSTRAIN(throttle_command->torque_request, MINIMUM_THROTTLE_COMMAND, MAXIMUM_THROTTLE_COMMAND);

    float spoof_voltage_low = THROTTLE_POSITION_TO_VOLTS_LOW(clamped_position);

    spoof_voltage_low =
      CONSTRAIN(spoof_voltage_low, THROTTLE_SPOOF_LOW_SIGNAL_VOLTAGE_MIN, THROTTLE_SPOOF_LOW_SIGNAL_VOLTAGE_MAX);

    float spoof_voltage_high = THROTTLE_POSITION_TO_VOLTS_HIGH(clamped_position);

    spoof_voltage_high =
      CONSTRAIN(spoof_voltage_high, THROTTLE_SPOOF_HIGH_SIGNAL_VOLTAGE_MIN, THROTTLE_SPOOF_HIGH_SIGNAL_VOLTAGE_MAX);
