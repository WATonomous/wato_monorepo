...
WHEN("^an enable brake command is received$")
{
  ...
  oscc_brake_enable_s * brake_enable = (oscc_brake_enable_s *)g_mock_mcp_can_read_msg_buf_buf;
...
WHEN("^a disable brake command is received$")
{
  ...
  oscc_brake_disable_s * brake_disable = (oscc_brake_disable_s *)g_mock_mcp_can_read_msg_buf_buf;
...
WHEN("^the brake pedal command (.*) is received$")
{
  ...
  oscc_brake_command_s * brake_command = (oscc_brake_command_s *)g_mock_mcp_can_read_msg_buf_buf;
...
THEN("^the brake pedal command should be parsed$")
{
  ...
  assert_that_double(g_brake_control_state.commanded_pedal_position, is_equal_to_double(brake_command->pedal_command));
}
...
THEN("^the (.*) solenoid should be activated with duty cycle (.*)$")
{
  ...
  if (solenoid == "ACCUMULATOR") {
    ...
    assert_that(g_mock_arduino_analog_write_pins[i], is_equal_to(PIN_RELEASE_SOLENOID_FRONT_LEFT));

    assert_that(g_mock_arduino_analog_write_val[i], is_equal_to(SOLENOID_PWM_OFF));

    ++i;

    assert_that(g_mock_arduino_analog_write_pins[i], is_equal_to(PIN_RELEASE_SOLENOID_FRONT_RIGHT));
    ...
    assert_that(g_mock_arduino_analog_write_pins[i], is_equal_to(PIN_ACCUMULATOR_SOLENOID_FRONT_LEFT));

    assert_that(g_mock_arduino_analog_write_val[i], is_equal_to(duty_cycle));

    ++i;

    assert_that(g_mock_arduino_analog_write_pins[i], is_equal_to(PIN_ACCUMULATOR_SOLENOID_FRONT_RIGHT));
