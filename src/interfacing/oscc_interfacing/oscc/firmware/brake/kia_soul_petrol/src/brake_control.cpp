...
{
  if (
    (g_brake_control_state.enabled == false) && (g_brake_control_state.operator_override == false) &&
...
void check_for_operator_override(void)
{
  ...
  if (g_brake_control_state.enabled == true || g_brake_control_state.operator_override == true) {
    ...
    if (operator_overridden == true) {
      ...
      DTC_CLEAR(g_brake_control_state.dtcs, OSCC_BRAKE_DTC_OPERATOR_OVERRIDE);
...
{
  if (
    (g_brake_control_state.enabled == true) || DTC_CHECK(g_brake_control_state.dtcs, OSCC_BRAKE_DTC_INVALID_SENSOR_VAL))
  {
    bool master_cylinder_pressure_fault = check_master_cylinder_pressure_sensor_for_fault();

    if (master_cylinder_pressure_fault == true) {
      DEBUG_PRINTLN("Bad value read from master cylinder pressure sensor");
    }

    bool accumulator_pressure_fault = check_accumulator_pressure_sensor_for_fault();

    if (accumulator_pressure_fault == true) {
      DEBUG_PRINTLN("Bad value read from accumulator pressure sensor");
    }

    bool wheel_pressure_fault = check_wheel_pressure_sensor_for_fault();
    ...
    if (
      (master_cylinder_pressure_fault == true) || (accumulator_pressure_fault == true) ||
      (wheel_pressure_fault == true))
    {
      disable_control();

      DTC_SET(g_brake_control_state.dtcs, OSCC_BRAKE_DTC_INVALID_SENSOR_VAL);

      publish_fault_report();
    } else {
      DTC_CLEAR(g_brake_control_state.dtcs, OSCC_BRAKE_DTC_INVALID_SENSOR_VAL);
    }
  }
}
...
void update_brake(void)
{
  if (g_brake_control_state.enabled == true) {
    ...
    pressure_at_wheels_target = interpolate(g_brake_control_state.commanded_pedal_position, &pressure_ranges);

    pressure_at_wheels_current = read_pressure_sensor();

    int16_t ret = pid_update(&g_pid, pressure_at_wheels_target, pressure_at_wheels_current, time_between_loops);
...
{
  cli();
  int raw_adc_front_left = analogRead(PIN_PRESSURE_SENSOR_FRONT_LEFT);

  int raw_adc_front_right = analogRead(PIN_PRESSURE_SENSOR_FRONT_RIGHT);
  sei();
...
static bool check_master_cylinder_pressure_sensor_for_fault(void)
{
  ...
  if ((master_cylinder_pressure_1 == 0) || (master_cylinder_pressure_2 == 0)) {
    ++fault_count;
...
static bool check_wheel_pressure_sensor_for_fault(void)
{
  ...
  // sensor pins tied to ground - a value of zero indicates disconnection
  if ((wheel_pressure_front_left == 0) || (wheel_pressure_front_right == 0)) {
    ++fault_count;
...
static void pressure_startup_check(void)
{
  ...
  if (
    (pressure_front_left < BRAKE_PRESSURE_SENSOR_CHECK_VALUE_MIN) ||
