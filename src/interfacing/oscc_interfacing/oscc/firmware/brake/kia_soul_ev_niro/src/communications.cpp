...
static void process_brake_command(const uint8_t * const data);
...
void publish_brake_report(void)
{
  ...
  cli();
  g_control_can.sendMsgBuf(OSCC_BRAKE_REPORT_CAN_ID, CAN_STANDARD, OSCC_BRAKE_REPORT_CAN_DLC, (uint8_t *)&brake_report);
  sei();
}
...
{
  if (data != NULL) {
    const oscc_brake_command_s * const brake_command = (oscc_brake_command_s *)data;

    const float clamped_position =
      CONSTRAIN(brake_command->pedal_command, MINIMUM_BRAKE_COMMAND, MAXIMUM_BRAKE_COMMAND);

    float spoof_voltage_low = BRAKE_POSITION_TO_VOLTS_LOW(clamped_position);

    spoof_voltage_low =
      CONSTRAIN(spoof_voltage_low, BRAKE_SPOOF_LOW_SIGNAL_VOLTAGE_MIN, BRAKE_SPOOF_LOW_SIGNAL_VOLTAGE_MAX);

    float spoof_voltage_high = BRAKE_POSITION_TO_VOLTS_HIGH(clamped_position);

    spoof_voltage_high =
      CONSTRAIN(spoof_voltage_high, BRAKE_SPOOF_HIGH_SIGNAL_VOLTAGE_MIN, BRAKE_SPOOF_HIGH_SIGNAL_VOLTAGE_MAX);
