...
static void read_brake_pedal_position_sensor(brake_pedal_position_s * const value);
...
void check_for_faults(void)
{
  ...
  if ((g_brake_control_state.enabled == true) || (g_brake_control_state.dtcs > 0)) {
    read_brake_pedal_position_sensor(&brake_pedal_position);

    uint32_t brake_pedal_position_average = (brake_pedal_position.low + brake_pedal_position.high) / 2;

    bool operator_overridden = condition_exceeded_duration(
      brake_pedal_position_average >= BRAKE_PEDAL_OVERRIDE_THRESHOLD, FAULT_HYSTERESIS, &operator_override_state);

    bool inputs_grounded = check_voltage_grounded(
      brake_pedal_position.high, brake_pedal_position.low, FAULT_HYSTERESIS, &grounded_fault_state);
...
void update_brake(uint16_t spoof_command_high, uint16_t spoof_command_low)
{
  if (g_brake_control_state.enabled == true) {
    uint16_t spoof_high =
      constrain(spoof_command_high, BRAKE_SPOOF_HIGH_SIGNAL_RANGE_MIN, BRAKE_SPOOF_HIGH_SIGNAL_RANGE_MAX);

    uint16_t spoof_low =
      constrain(spoof_command_low, BRAKE_SPOOF_LOW_SIGNAL_RANGE_MIN, BRAKE_SPOOF_LOW_SIGNAL_RANGE_MAX);

    if ((spoof_high > BRAKE_LIGHT_SPOOF_HIGH_THRESHOLD) || (spoof_low > BRAKE_LIGHT_SPOOF_LOW_THRESHOLD)) {
      cli();
...
{
  if (g_brake_control_state.enabled == false && g_brake_control_state.operator_override == false) {
    const uint16_t num_samples = 20;
    prevent_signal_discontinuity(
      g_dac, num_samples, PIN_BRAKE_PEDAL_POSITION_SENSOR_LOW, PIN_BRAKE_PEDAL_POSITION_SENSOR_HIGH);
