...
void brake_report_callback(oscc_brake_report_s *report) {
  if (report->operator_override && g_node_instance) {
    ...
    g_node_instance->latest_override_ =
        OsccInterfacingNode::OverrideType::BRAKE;
...
void throttle_report_callback(oscc_throttle_report_s *report) {
  if (report->operator_override && g_node_instance) {
    // Async-signal-safe: only atomic operations allowed
    g_node_instance->latest_override_ =
        OsccInterfacingNode::OverrideType::THROTTLE;
    g_node_instance->has_override_.store(true);
  }
}
...
void steering_report_callback(oscc_steering_report_s *report) {
  if (report->operator_override && g_node_instance) {
    // Async-signal-safe: only atomic operations allowed
    g_node_instance->latest_override_ =
        OsccInterfacingNode::OverrideType::STEERING;
    g_node_instance->has_override_.store(true);
  }
}
...
void fault_report_callback(oscc_fault_report_s *report) {
  if (g_node_instance) {
    ...
    if (report->fault_origin_id == FAULT_ORIGIN_BRAKE) {
      g_node_instance->latest_fault_ =
          OsccInterfacingNode::FaultType::BRAKE_FAULT;
    } else if (report->fault_origin_id == FAULT_ORIGIN_STEERING) {
      g_node_instance->latest_fault_ =
          OsccInterfacingNode::FaultType::STEERING_FAULT;
    } else if (report->fault_origin_id == FAULT_ORIGIN_THROTTLE) {
      g_node_instance->latest_fault_ =
          OsccInterfacingNode::FaultType::THROTTLE_FAULT;
    }
...
void OsccInterfacingNode::configure() {
  ...
  is_armed_publish_rate_hz =
      this->get_parameter("is_armed_publish_rate_hz").as_int();
  oscc_can_bus_ = this->get_parameter("oscc_can_bus").as_int();
  steering_scaling_ = this->get_parameter("steering_scaling").as_double();
  disable_boards_on_fault_ =
      this->get_parameter("disable_boards_on_fault").as_bool();
  steering_conversion_factor_ =
      this->get_parameter("steering_conversion_factor").as_double();
  ...
  roscco_sub_ = this->create_subscription<roscco_msg::msg::Roscco>(
      "/roscco", rclcpp::QoS(1),
      std::bind(&OsccInterfacingNode::roscco_callback, this,
                std::placeholders::_1));

  // Create publishers
  is_armed_pub_ = this->create_publisher<std_msgs::msg::Bool>(
      "/oscc_interfacing/is_armed", rclcpp::QoS(1));

  wheel_speeds_pub_ = this->create_publisher<roscco_msg::msg::WheelSpeeds>(
      "/oscc_interfacing/wheel_speeds", rclcpp::QoS(1));

  steering_angle_pub_ = this->create_publisher<roscco_msg::msg::SteeringAngle>(
      "/oscc_interfacing/steering_angle", rclcpp::QoS(1));

  // Create arm service
  arm_service_ = this->create_service<std_srvs::srv::SetBool>(
      "/oscc_interfacing/arm",
      std::bind(&OsccInterfacingNode::arm_service_callback, this,
  ...
  data_process_timer_ = this->create_wall_timer(
      ...
      std::bind(&OsccInterfacingNode::process_queued_data, this));

  RCLCPP_INFO(
      this->get_logger(),
      "OsccInterfacingNode configured: armed=%d, is_armed_publish_rate=%d Hz",
      is_armed_, is_armed_publish_rate_hz);
  ...
  if (oscc_open(oscc_can_bus_) != OSCC_OK) {
    ...
    if (oscc_subscribe_to_steering_reports(steering_report_callback) !=
        OSCC_OK) {
      RCLCPP_ERROR(this->get_logger(),
                   "Failed to subscribe to steering reports");
    }
    if (oscc_subscribe_to_throttle_reports(throttle_report_callback) !=
        OSCC_OK) {
      RCLCPP_ERROR(this->get_logger(),
                   "Failed to subscribe to throttle reports");
    }
...
void OsccInterfacingNode::roscco_callback(
    const roscco_msg::msg::Roscco::ConstSharedPtr msg) {
  // if not armed ignore
  {
    std::lock_guard<std::mutex> lock(arm_mutex_);
    if (!is_armed_) {
      RCLCPP_WARN(this->get_logger(),
                  "Vehicle not armed, Ignoring roscco message");
      return;
    }
  }
  ...
  if ((now - last_message_time_).nanoseconds() <
      40000000) { // 40ms in nanoseconds
    RCLCPP_WARN(
        this->get_logger(),
        "Message too soon, Ignoring roscco message to avoid CAN overload");
    return;
  }
...
void OsccInterfacingNode::publish_wheel_speeds(float NE, float NW, float SE,
                                               float SW) {
  roscco_msg::msg::WheelSpeeds msg;
  msg.ne = NE;
  msg.nw = NW;
...
oscc_result_t OsccInterfacingNode::handle_any_errors(oscc_result_t result) {
  ...
  RCLCPP_ERROR(this->get_logger(),
               "Error from OSCC API: %d, ATTEMPTING TO DISARM ALL BOARDS",
               result);
  // Attempt to disarm all boards
  if (oscc_disable() != OSCC_OK) {
    RCLCPP_FATAL(
        this->get_logger(),
        "!! FAILED TO DISARM ALL BOARDS, MANUAL INTERVENTION REQUIRED !!");
    RCLCPP_FATAL(
        this->get_logger(),
        "!! FAILED TO DISARM ALL BOARDS, MANUAL INTERVENTION REQUIRED !!");
    RCLCPP_FATAL(
    ...
    RCLCPP_INFO(this->get_logger(),
                "All boards disarmed successfully after error");
  }
