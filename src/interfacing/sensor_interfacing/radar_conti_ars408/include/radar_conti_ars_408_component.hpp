...
class radar_conti_ars408 : public rclcpp_lifecycle::LifecycleNode {
 public:
  ...
  template <typename T>
  void declare_parameter_with_type(rclcpp_lifecycle::LifecycleNode::SharedPtr node,
  ...
  template <typename T>
  void get_parameter_with_type(rclcpp_lifecycle::LifecycleNode::SharedPtr node,
                               const std::string &param_name, T &value) {
    if constexpr (std::is_same_v<T, int>) {
  ...
  static constexpr double covariance[] = {
      0.005,  0.007,  0.010,  0.014,  0.020,  0.029,  0.041,  0.058,   0.082,   0.116,  0.165,
      0.234,  0.332,  0.471,  0.669,  0.949,  1.346,  1.909,  2.709,   3.843,   5.451,  7.734,
      10.971, 15.565, 22.081, 31.325, 44.439, 63.044, 89.437, 126.881, 180.000, 200.000};
 ...
 private:
  ...
  std::vector<
      rclcpp_lifecycle::LifecyclePublisher<radar_conti_ars408_msgs::msg::FilterStateCfg>::SharedPtr>
      filter_config_publishers_;
  rclcpp_lifecycle::LifecyclePublisher<radar_msgs::msg::RadarPacket>::SharedPtr
  ...
  // update filter
  bool setFilter(const int &sensor_id, const int &active, const int &valid, const int &type,
                 const int &index, const int &min_value, const int &max_value);
