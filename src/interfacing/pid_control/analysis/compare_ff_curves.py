...
fig1.add_trace(go.Surface(
    x=v_grid, y=d_grid, z=T_base,
    colorscale=[[0, "rgb(40,40,40)"], [1, "rgb(40,40,40)"]],
    opacity=0.8, showscale=False, name="Baseline (100%)",
...
for i, s in enumerate(scales):
    ...
    fig.add_trace(go.Surface(
        ...
        opacity=0.3, showscale=False, name="Baseline (100%)",
    ))
    ...
    fig.add_trace(go.Surface(
        x=v_grid, y=d_grid, z=T_grid - T_base,
        colorscale="RdBu", opacity=0.5, showscale=False,
        name="Difference from baseline", visible="legendonly",
    ))
...
fig3.add_trace(go.Surface(
    x=v_grid, y=d_grid, z=T_base,
    colorscale=[[0, "rgb(40,40,40)"], [1, "rgb(40,40,40)"]],
    opacity=0.7, showscale=False, name="Baseline (100%)",
))
...
fig3.update_layout(
    ...
    scene=dict(xaxis_title="Velocity (m/s)", yaxis_title="Steering Angle δ (rad)", zaxis_title="Torque"),
    width=1200, height=900,
)
...
for i, s in enumerate(scales):
    c = [ci * s for ci in BASELINE_COEFFS]
    f = BASELINE_FRICTION * s
    print(f"Curve {i+1:02d}     {s*100:5.0f}%  {c[0]:>10.6f} {c[1]:>12.6f} {c[2]:>12.6f} {f:>12.6f}")
print("-" * 70)
print(f"Baseline      100%  {BASELINE_COEFFS[0]:>10.6f} {BASELINE_COEFFS[1]:>12.6f} {BASELINE_COEFFS[2]:>12.6f} {BASELINE_FRICTION:>12.6f}")
