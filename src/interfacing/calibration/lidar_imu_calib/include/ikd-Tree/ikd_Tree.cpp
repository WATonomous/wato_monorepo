...
void KD_TREE<PointType>::start_thread()
{
  ...
  pthread_create(&rebuild_thread, NULL, multi_thread_ptr, reinterpret_cast<void *>(this));
...
void KD_TREE<PointType>::Nearest_Search(
  PointType point, int k_nearest, PointVector & Nearest_Points, vector<float> & Point_Distance, float max_dist)
{
...
bool KD_TREE<PointType>::Criterion_Check(KD_TREE_NODE * root)
{
  ...
  delete_evaluation = static_cast<float>(root->invalid_point_num) / root->TreeSize;
  balance_evaluation = static_cast<float>(son_ptr->TreeSize) / (root->TreeSize - 1);
...
void KD_TREE<PointType>::Update(KD_TREE_NODE * root)
{
  ...
  if (root == Root_Node && root->TreeSize > 3) {
    KD_TREE_NODE * son_ptr = root->left_son_ptr;
    if (son_ptr == nullptr) son_ptr = root->right_son_ptr;
    float tmp_bal = static_cast<float>(son_ptr->TreeSize) / (root->TreeSize - 1);
    root->alpha_del = static_cast<float>(root->invalid_point_num) / root->TreeSize;
