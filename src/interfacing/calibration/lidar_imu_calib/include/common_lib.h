...
#include "lidar_imu_calib/msg/states.hpp"
...
typedef lidar_imu_calib::msg::Pose6D Pose6D;
...
bool esti_plane(Matrix<T, 4, 1> & pca_result, const PointVector & point, const T & threshold)
{
  ...
  for (int j = 0; j < NUM_MATCH_POINTS; j++) {
    if (
      fabs(pca_result(0) * point[j].x + pca_result(1) * point[j].y + pca_result(2) * point[j].z + pca_result(3)) >
      threshold)
    {
      return false;
    }
