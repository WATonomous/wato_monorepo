...
#include "lidar_imu_calib/msg/pose6_d.hpp"
...
void ImuProcess::IMU_init(const MeasureGroup & meas, StatesGroup & state_inout, int & N)
{
  ...
  printf("IMU Initializing: %.1f %%", static_cast<double>(N) / MAX_INI_COUNT * 100);
  V3D cur_acc, cur_gyr;
...
void ImuProcess::Forward_propagation_without_imu(
  const MeasureGroup & meas, StatesGroup & state_inout, PointCloudXYZI & pcl_out)
{
  pcl_out = *(meas.lidar);
  /*** sort point clouds by offset time ***/
  const double & pcl_beg_time = meas.lidar_beg_time;
  sort(pcl_out.points.begin(), pcl_out.points.end(), time_list);
  const double & pcl_end_offset_time = pcl_out.points.back().curvature / static_cast<double>(1000);
  ...
  for (; it_pcl != pcl_out.points.begin(); it_pcl--) {
    dt_j = pcl_end_offset_time - it_pcl->curvature / static_cast<double>(1000);
...
void ImuProcess::propagation_and_undist(const MeasureGroup & meas, StatesGroup & state_inout, PointCloudXYZI & pcl_out)
{
  ...
  /*** sort point clouds by offset time ***/
  sort(pcl_out.points.begin(), pcl_out.points.end(), time_list);
  pcl_end_time = pcl_beg_time + pcl_out.points.back().curvature / static_cast<double>(1000);
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
    ...
    fout_imu << setw(10) << get_time_sec(head->header.stamp) << "  " << angvel_now.transpose() << " "
  ...
  for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--) {
    ...
    for (; it_pcl->curvature / static_cast<double>(1000) > head->offset_time; it_pcl--) {
      dt = it_pcl->curvature / static_cast<double>(1000) - head->offset_time;  // dt = t_j - t_i > 0
      ...
      V3D P_compensate =
        state_inout.offset_R_L_I.transpose() *
        (state_inout.rot_end.transpose() *
           (R_i * (state_inout.offset_R_L_I * p_in + state_inout.offset_T_L_I) + P_i - state_inout.pos_end) -
    ...
    if (imu_need_init_) {
      if (!LI_init_done) {
        ...
        if (init_iter_num > MAX_INI_COUNT) {
          ...
          printf(
            "IMU Initialization Done: Gravity: %.4f %.4f %.4f, Acc norm: %.4f",
            stat.gravity[0],
            stat.gravity[1],
            stat.gravity[2],
            mean_acc.norm());
