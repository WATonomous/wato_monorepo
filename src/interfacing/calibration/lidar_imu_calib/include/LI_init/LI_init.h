...
struct Angular_Vel_Cost_only_Rot
{
  ...
  : IMU_ang_vel(IMU_ang_vel_)
  , Lidar_ang_vel(Lidar_ang_vel_)
  {}
...
struct Angular_Vel_Cost
{
  ...
  static ceres::CostFunction * Create(
    const V3D IMU_ang_vel_, const V3D IMU_ang_acc_, const V3D Lidar_ang_vel_, const double deltaT_LI_)
  {
...
struct Linear_acc_Cost
{
  ...
  : LidarState(LidarState_)
  , R_LI(R_LI_)
  , IMU_linear_acc(IMU_linear_acc_)
  {}
  ...
  bool operator()(const T * q, const T * b_a, const T * trans, T * residual) const
  {
    ...
    Eigen::Matrix<T, 3, 1> resi = R_LL0_T * R_LI_T_transpose * IMU_linear_acc_T - R_LL0_T * bias_aL + R_GL0 * STD_GRAV -
                                  Lidar_linear_acc_T - R_LL0_T * Jacob_trans_T * T_IL;
