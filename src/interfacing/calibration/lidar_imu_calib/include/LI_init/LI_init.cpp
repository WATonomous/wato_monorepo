...
void LI_Init::downsample_interpolate_IMU(const double & move_start_time)
{
  ...
  for (int i = 0; i < Lidar_state_group.size(); i++) {
    for (size_t j = 1; j < IMU_state_group_ALL.size(); j++) {
      if (
        IMU_state_group_ALL[j - 1].timeStamp <= Lidar_state_group[i].timeStamp &&
        IMU_state_group_ALL[j].timeStamp > Lidar_state_group[i].timeStamp)
      {
        ...
        push_IMU_CalibState(
          IMU_state_interpolation.ang_vel, IMU_state_interpolation.linear_acc, Lidar_state_group[i].timeStamp);
        break;
      }
    }
  }
...
void LI_Init::solve_Rotation_only()
{
  ...
  for (size_t i = 0; i < IMU_state_group.size(); i++) {
    ...
    problem_rot.AddResidualBlock(
      Angular_Vel_Cost_only_Rot::Create(IMU_state_group[i].ang_vel, Lidar_state_group[i].ang_vel), nullptr, R_LI_quat);
  }
  ...
  for (int i = 0; i < IMU_state_group.size(); i++) {
    ...
    problem_ang_vel.AddResidualBlock(
      Angular_Vel_Cost::Create(
        IMU_state_group[i].ang_vel, IMU_state_group[i].ang_acc, Lidar_state_group[i].ang_vel, deltaT),
      nullptr,
      R_LI_quat,
      bias_g,
      &time_lag2);
...
void LI_Init::solve_trans_biasacc_grav()
{
  ...
  for (size_t i = 0; i < IMU_state_group.size(); i++) {
    problem_acc.AddResidualBlock(
      Linear_acc_Cost::Create(Lidar_state_group[i], Rot_Lidar_wrt_IMU, IMU_state_group[i].linear_acc),
      nullptr,
      R_GL0_quat,
      bias_aL,
      Trans_IL);
...
bool LI_Init::data_sufficiency_assess(
  MatrixXd & Jacobian_rot, int & frame_num, V3D & lidar_omg, int & orig_odom_freq, int & cut_frame_num)
{
  ...
  if (frame_num % orig_odom_freq * cut_frame_num == 0) {
    ...
    V3D Rot_percent(
      Scaled_Eigen[1] * Scaled_Eigen[2], Scaled_Eigen[0] * Scaled_Eigen[2], Scaled_Eigen[0] * Scaled_Eigen[1]);

    V3D Rot_percent_scaled(
      Rot_percent[0] < 0.99 ? Rot_percent[0] : 1,
...
void LI_Init::printProgress(double percentage, int axis_ascii)
{
  ...
  if (percentage < 1) {
    printf(BOLDYELLOW "Rotation around Lidar %c Axis: ", static_cast<char>(axis_ascii));
    printf(YELLOW "%3d%% [%.*s%*s]\n", val, lpad, PBSTR, rpad, "");
    cout << RESET;
  } else {
    printf(BOLDGREEN "Rotation around Lidar %c Axis: ", static_cast<char>(axis_ascii));
    printf(GREEN "%3d%% [%.*s%*s]\n", val, lpad, PBSTR, rpad, "");
    cout << RESET;
  }
}
...
void LI_Init::LI_Initialization(
  int & orig_odom_freq, int & cut_frame_num, double & timediff_imu_wrt_lidar, const double & move_start_time)
{
...
void LI_Init::print_initialization_result(
  double & time_L_I, M3D & R_L_I, V3D & p_L_I, V3D & bias_g, V3D & bias_a, V3D gravity)
{
  cout.setf(ios::fixed);
