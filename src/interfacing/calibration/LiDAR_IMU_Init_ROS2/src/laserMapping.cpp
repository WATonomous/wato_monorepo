...
void print_refine_result()
{
    ...
    printf(BOLDCYAN "[URDF] " RESET);
    cout << "xyz=\"" << state.offset_T_L_I(0) << " " << state.offset_T_L_I(1) << " " << state.offset_T_L_I(2) << "\"" << endl;
    printf(BOLDCYAN "[URDF] " RESET);
...
public:
    LaserMappingNode(const rclcpp::NodeOptions& options = rclcpp::NodeOptions()) : Node("laser_mapping", options)
    {
        ...
        if (skip_init) {
            ...
            Eigen::Matrix3d R_LI;
            R_LI = Eigen::AngleAxisd(yaw,   Eigen::Vector3d::UnitZ())
...
#pragma omp parallel for
        ...
        if (!imu_en && !data_accum_start && state.pos_end.norm() > 0.05)
        {
            if (skip_init)
            {
                ...
                {
                    imu_buffer[i]->header.stamp = rclcpp::Time(
                        rclcpp::Time(imu_buffer[i]->header.stamp).seconds() - time_lag_IMU_wtr_lidar);
                }
