...
TEST_CASE_METHOD(TestExecutorFixture, "PidControlRawNode Basic Operation", "[pid_control_raw]")
{
  ...
  auto pid_node = std::make_shared<pid_control_raw::PidControlRawNode>(options);
  add_node(pid_node);
  ...
  // racing set_parameter() on the test thread with compute() on the executor thread.
  ...
  SECTION("Verify output saturation behavior")
  {
    // Steering: 0.5 clamp
    pid_node->set_parameter(rclcpp::Parameter("steering_pid.p", 0.0));
